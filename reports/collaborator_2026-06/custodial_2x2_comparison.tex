...
\textbf{Two caveats on the OURS custodial panels (so they are not over-read).}
\begin{enumerate}\setlength{\itemsep}{0.3em}\leftskip-0.4em
\item \emph{The $Z\to b\bar b$ custodial collapse ($\delta g_R^b\!\to\!0$) is
representation-specific.} It is exact only for $b_R\in(1,1)_{-1/3}$, where
$T^3_L{=}T^3_R{=}0$ gives $P_C$ protection (ACDP Table~1, row~2). Other $b_R$
embeddings give a \emph{nonzero} $\delta g_R^b$ of either sign (e.g.\ the
$(2,4)$-type used to fit $A_{FB}^b$). Our panel pins the protected embedding; it
is a model choice, not a theorem.
\item \emph{The ``$\varepsilon_K$ identical'' claim is exact for the dominant
operator, not for every flavor observable.} The $\varepsilon_K$ \emph{wall} is
genuinely custodial-blind --- it is set by the color-octet KK-gluon $C_4$, an EW
singlet (and Blanke's full custodial $\Delta F{=}2$ confirms it). But custodial
\emph{does} touch flavor at subleading order through the EW KK bosons (Blanke:
$Z_H,Z'$ can compete with the KK gluon in $B$-mixing) and custodian loops
($b\to s\gamma$). Our \texttt{deltaf2} path has no $\mathrm{ew\_model}$ branch, so
``custodial changes \emph{nothing} in flavor'' is a leading-order statement; what
is rigorous is that the binding $\varepsilon_K$ wall does not move.
\end{enumerate}
