...
\textbf{Lane framing (important).} The reproductions in this report are the
\textbf{anarchic baseline (lane A)} --- the literature strawman --- used to validate
the code against published figures. They are \textbf{NOT the production / our model}:
production as run (lane B, the fit-aligned MFV without V5KM) is a sharp
$\varepsilon_K$ wall at $\approx$\textbf{7~TeV}, and the FPR ideal (lane C, V5KM
aligned) is $\approx$\textbf{2~TeV}. So the ${\sim}30$~TeV anarchic $\varepsilon_K$
cloud below is the strawman, not ``our floor''. Canonical three-lane definitions:
...
The $\varepsilon_K$ ``typical'' floor in the table below is \textbf{lane A
(anarchic)} --- the literature strawman, NOT our model. Production as run (lane B)
is a sharp wall at $\approx$7~TeV, and the FPR ideal (lane C, V5KM) is
$\approx$2~TeV (see \texttt{docs/MODEL\_CONVENTIONS.md}).
...
\midrule
$\varepsilon_K$            & ${\sim}30$~TeV (lane A anarchic; lane B prod.\ ${\sim}7$, lane C FPR ${\sim}2$) & ${\lesssim}1$~TeV & yes (align $\mathrm{Im}\,M_{12}\!\to\!0$) \\
