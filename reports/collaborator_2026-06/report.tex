...
Residual: our overall consistent fraction (${\sim}40\%$) is ${\sim}2\times$ higher
than Bauer's 19\% because the lower (aligned) tail of our cloud is
over-populated; the spread, floor, axes, coloring and scenario ordering all
match. A companion $2\times2$ S1--S4 panel mirrors the paper's full layout (S3
...
The paper's anarchic cloud sits near the SM point $(1, 0)$ but with a genuine wide
scatter. With the matched ensemble ($Y_{\max}=3$, scanned $c$) our cloud now
reproduces that spread: $C_{B_d}$ median $1.02$ with $p5 = 0.89$ (= Bauer's
$C_{B_d} = 0.89\pm0.17$), $\phi_{B_d}$ centered at $0$ with tens-of-degrees tails
--- the full anarchic cloud rather than the artificially narrow locus of the old
$|Y|<2.1$ generator.
...
$C_{B_s}$ median $1.04$, $p5 = 0.94$ (= Bauer's $C_{B_s} = 0.93\pm0.19$),
$\phi_{B_s}$ centered at $0$ with broad tails, and $S_{\psi\phi}$ spanning
$[-0.57,\,0.04,\,0.62]$ --- reproducing Bauer's ``large $\phi_{B_s}$ allowed''
(the old narrow cloud could not show this).
