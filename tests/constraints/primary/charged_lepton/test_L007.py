"""Production tests for L007 (tau -> mu gamma)."""
...
_SIDECAR = _REPO_ROOT / "flavor_catalog" / "processes" / "charged_lepton" / "L007.yaml"
_L001_SIDECAR = (
...
def _tau_mu_rotation_pmns() -> np.ndarray:
    theta = 0.5
    c = math.cos(theta)
    s = math.sin(theta)
    return np.asarray(
...
def _proxy_point(
    ...
    *,
    ...
    proxy = tau_to_mu_gamma_proxy_input(
        y_n_bar,
        _tau_mu_rotation_pmns(),
        m_kk_gev,
        source="L007 test proxy",
    )
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(tau -> mu gamma)"
...
def test_anchor_matches_yaml_and_loud_fail_probe():
    ...
    belle = pdg["primary_experiment"]
    babar = pdg["prior_experimental_limit"]
    ...
    assert constraint.anchor.primary_experiment.value == pytest.approx(belle["value"])
    assert constraint.anchor.primary_experiment.source_url == belle["source_url"]
    assert constraint.anchor.prior_experimental_limit.value == pytest.approx(
        babar["value"]
    )
    assert constraint.anchor.prior_experimental_limit.source_url == babar["source_url"]
    ...
    assert constraint.anchor.budget == pytest.approx(4.2e-8)
...
def test_tau_to_mu_gamma_rejects_caller_flavor_override():
    constraint = fcc.get(_PID)
    bad = {
        "initial_flavor": "mu",
        "final_flavor": "e",
        "y_n_bar": [0.1, 0.2, 0.3],
        "pmns": _tau_mu_rotation_pmns(),
        "m_kk_gev": 3000.0,
...
def test_proxy_numerics_match_mu_to_e_core_with_tau_mu_rotation():
    constraint = fcc.get(_PID)
    ...
    expected_off_23 = complex(core["off_diagonal_12"])
    expected_off_12 = complex(core["product_matrix"][2, 0])
    ...
    assert result.diagnostics["off_diagonal_23"] == pytest.approx(expected_off_23)
    assert result.diagnostics["product_matrix"][1][2] == pytest.approx(expected_off_23)
    ...
    assert result.diagnostics["row_permutation"] == (1, 2, 0)
    assert result.diagnostics["final_initial_indices_zero_based"] == (1, 2)
...
def test_evaluate_runs_end_to_end_with_real_finite_fields_and_complex_diagnostics():
    ...
    assert isinstance(result.diagnostics["off_diagonal_23"], complex)
    assert isinstance(result.diagnostics["off_diagonal_32"], complex)
    assert isinstance(result.diagnostics["product_matrix"][1][2], complex)
    for key in (
        "dipole_lhs",
        "dipole_rhs",
    ...
    assert result.diagnostics["input_kind"] == "TauToMuGammaProxyInput"
