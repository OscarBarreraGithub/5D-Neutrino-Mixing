"""Production tests for T007 (top FCNC t -> H c)."""
...
from flavor_catalog_constraints.primary.top_higgs_ew import T007 as t007_module
from quarkConstraints.couplings import QuarkMassBasisCouplings
from quarkConstraints.top_fcnc import higgs_scalar_branching_fraction

_PID = "T007"
_REPO_ROOT = Path(__file__).resolve().parents[4]
_SIDECAR = _REPO_ROOT / "flavor_catalog" / "processes" / "top_higgs_ew" / "T007.yaml"
...
def _core_t_to_c_higgs_recomputation(
    *,
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(t -> H c)"


def test_anchor_matches_yaml_and_loud_fail_probe():
    constraint = fcc.get(_PID)
    entries = _yaml_entries_by_id()
    pdg = entries["PDG2026:T007:tHc_combined"]
    atlas_multilepton = entries["ATLAS2024:T007:tHc_multilepton"]
    atlas_combined = entries["ATLAS2024:T007:tHc_combined"]
    cms_samesign = entries["CMS2025:T007:tHc_samesign"]
    cms_combined = entries["CMS2025:T007:tHc_combined"]

    assert constraint.anchor.pdg_thc_combined.value == pytest.approx(
        _parse_yaml_number(pdg["value"])
    )
    assert constraint.anchor.pdg_thc_combined.source_url == pdg["source_url"]
    assert constraint.anchor.atlas_thc_multilepton.value == pytest.approx(
        _parse_yaml_number(atlas_multilepton["value"])
    )
    assert constraint.anchor.atlas_thc_combined.value == pytest.approx(
        _parse_yaml_number(atlas_combined["value"])
    )
    assert constraint.anchor.cms_thc_samesign.value == pytest.approx(
        _parse_yaml_number(cms_samesign["normalized_value"])
    )
    assert constraint.anchor.cms_thc_combined.value == pytest.approx(
        _parse_yaml_number(cms_combined["normalized_value"])
    )
    assert constraint.anchor.budget == pytest.approx(_parse_yaml_number(pdg["value"]))
    assert constraint.anchor.budget == pytest.approx(3.4e-4)
    assert constraint.anchor.active_limit.value_id == "PDG2026:T007:tHc_combined"

    with pytest.raises(AnchorError):
        t007_module._load_limit_anchor("PDG2026:T007:not_present", process_id=_PID)


def test_value_list_anchors_route_through_scaffold_load_anchor(monkeypatch):
    calls: list[tuple[str, ...]] = []
    original_load_anchor = t007_module.load_anchor

    def spy_load_anchor(*args, **kwargs):
        calls.append(tuple(kwargs["candidates"]))
        return original_load_anchor(*args, **kwargs)

    monkeypatch.setattr(t007_module, "load_anchor", spy_load_anchor)
    anchor = t007_module._load_t007_anchor(_PID)
    ...
    monkeypatch.setattr(t007_module, "load_anchor", mismatched_load_anchor)
    with pytest.raises(AnchorError, match="load_anchor selected"):
        t007_module._load_limit_anchor(
            "PDG2026:T007:tHc_combined",
            process_id=_PID,
        )
...
def test_zero_yukawa_limit_and_core_width_recomputation():
    ...
    point = point_builder.make_point(quark_mass_basis_couplings=couplings)
    result = constraint.evaluate(point)
    core = _core_t_to_c_higgs_recomputation(
        couplings=couplings,
...
def test_safe_point_passes_and_large_np_point_fails():
    ...
    safe = constraint.evaluate(
        point_builder.make_point(quark_mass_basis_couplings=_ct_couplings(left=10.0))
    )
    excluded = constraint.evaluate(
        point_builder.make_point(quark_mass_basis_couplings=_ct_couplings(left=11.0))
    )
...
def test_optional_kk_ew_mass_extra_changes_proxy_scale():
    couplings = _ct_couplings(left=11.0)
...
def test_evaluate_has_real_finite_fields_and_complex_diagnostics():
    result = fcc.get(_PID).evaluate(
        point_builder.make_point(
            quark_mass_basis_couplings=_ct_couplings(
                left=1.0e-1 + 2.0e-2j,
                right=5.0e-2j,
            )
        )
    ...
    assert result.diagnostics["light_quark"] == "c"
    assert result.diagnostics["light_up_index"] == 1
    assert "NEEDS-HUMAN-PHYSICS" in result.diagnostics["needs_human_physics"]
