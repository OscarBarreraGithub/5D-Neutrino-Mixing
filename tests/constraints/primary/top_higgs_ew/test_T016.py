...
def _proxy_point(
    ...
    right_etau_overlap: complex = 0.0j,
    *,
    m_kk_gev: float = 3000.0,
):
    proxy = zpole_lfv_etau_proxy_input(
        left_etau_overlap,
        right_etau_overlap,
        m_kk_gev,
        source="T016 test proxy",
    )
...
def _manual_lfv_br(
    ...
    *,
    ...
    delta_left = complex(inputs.proxy_strength * scale * left_etau_overlap)
    delta_right = complex(inputs.proxy_strength * scale * right_etau_overlap)
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(Z -> e tau)"
...
def test_proxy_numerics_match_independent_effective_coupling_recomputation():
    ...
    assert result.diagnostics["delta_g_left_etau"] == pytest.approx(delta_left)
    assert result.diagnostics["delta_g_right_etau"] == pytest.approx(delta_right)
    ...
    assert "e+- tau-+" in result.diagnostics["branching_formula"]
    assert "NEEDS-HUMAN-PHYSICS" in result.diagnostics["needs_human_physics"]
    assert "NEEDS-HUMAN-PHYSICS" in result.diagnostics["z_lfv_etau_proxy"]["matching_assumption"]


def test_matrix_proxy_uses_e_tau_offdiagonal_entry():
    left_matrix = [[0.0, 8.0, 0.30], [0.0, 0.0, 9.0], [0.0, 0.0, 0.0]]
    right_matrix = [[0.0, 4.0, 0.02j], [0.0, 0.0, 5.0], [0.0, 0.0, 0.0]]
    point = point_builder.make_point(
        lepton_mass_basis_couplings={
            ...
            "m_kk_gev": 3000.0,
            "source": "T016 matrix proxy",
        }
    )
    result = fcc.get(_PID).evaluate(point)
    expected_br, delta_left, delta_right, norm, _ = _manual_lfv_br(0.30, 0.02j)

    assert result.predicted == pytest.approx(expected_br)
    assert result.diagnostics["delta_g_left_etau"] == pytest.approx(delta_left)
    assert result.diagnostics["delta_g_right_etau"] == pytest.approx(delta_right)
    assert result.diagnostics["effective_coupling_norm"] == pytest.approx(norm)
    assert result.diagnostics["z_lfv_etau_proxy"]["left_etau_overlap"] == pytest.approx(0.30)
...
def test_optional_kk_ew_mass_extra_changes_proxy_scale():
    ...
    assert default_result.diagnostics["z_lfv_etau_proxy"]["m_kk_gev"] == pytest.approx(3000.0)
    assert heavy_result.diagnostics["z_lfv_etau_proxy"]["m_kk_gev"] == pytest.approx(6000.0)
    assert heavy_result.diagnostics["kk_ew_mass_extra_used"] is True
    assert heavy_result.diagnostics["delta_g_left_etau"] == pytest.approx(
        default_result.diagnostics["delta_g_left_etau"] / 4.0
    )
