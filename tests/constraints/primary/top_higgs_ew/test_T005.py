"""Production tests for T005 (top FCNC t -> c g)."""
...
from flavor_catalog_constraints.primary.top_higgs_ew import T005 as t005_module
from quarkConstraints.couplings import QuarkMassBasisCouplings

_PID = "T005"
_REPO_ROOT = Path(__file__).resolve().parents[4]
_SIDECAR = _REPO_ROOT / "flavor_catalog" / "processes" / "top_higgs_ew" / "T005.yaml"
...
def _manual_t_to_c_gluon_proxy_br(
    *,
    ...
    partial_width = (
        ...
        * inputs.alpha_s_mt
        * inputs.m_top_gev
        * (abs(dipole_left) ** 2 + abs(dipole_right) ** 2)
    )
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(t -> c g)"


def test_anchor_matches_yaml_and_loud_fail_probe():
    constraint = fcc.get(_PID)
    entries = _yaml_entries_by_id()
    atlas_tcg = entries["PDG2025:T005:ATLAS2022:t_cg"]
    atlas_cugct = entries["PDG2025:T005:ATLAS2022:CuGct"]
    atlas_xsec = entries["PDG2025:T005:ATLAS2022:cg_to_t_cross_section"]
    cms_tcg = entries["CMS2017:T005:t_cg"]
    cms_kappa = entries["CMS2017:T005:kappa_tcg"]
    sm = entries["AguilarSaavedra2004:T005:SM"]

    assert constraint.anchor.atlas_tcg.value == pytest.approx(
        _parse_yaml_number(atlas_tcg["value"])
    )
    assert constraint.anchor.atlas_tcg.source_url == atlas_tcg["source_url"]
    assert constraint.anchor.atlas_cugct.value == pytest.approx(
        _parse_yaml_number(atlas_cugct["value"])
    )
    assert constraint.anchor.atlas_cg_to_t_cross_section.value == pytest.approx(
        _parse_yaml_number(atlas_xsec["value"])
    )
    assert constraint.anchor.cms_tcg.value == pytest.approx(
        _parse_yaml_number(cms_tcg["value"])
    )
    assert constraint.anchor.cms_kappa_tcg.value == pytest.approx(
        _parse_yaml_number(cms_kappa["value"])
    )
    assert constraint.anchor.sm_value == pytest.approx(_parse_yaml_number(sm["value"]))
    assert constraint.anchor.budget == pytest.approx(3.7e-4)
    assert constraint.anchor.active_limit.value_id == "PDG2025:T005:ATLAS2022:t_cg"

    with pytest.raises(AnchorError):
        t005_module._load_limit_anchor("PDG2025:T005:not_present", process_id=_PID)


def test_value_list_anchors_route_through_scaffold_load_anchor(monkeypatch):
    calls: list[tuple[str, ...]] = []
    original_load_anchor = t005_module.load_anchor

    def spy_load_anchor(*args, **kwargs):
        calls.append(tuple(kwargs["candidates"]))
        return original_load_anchor(*args, **kwargs)

    monkeypatch.setattr(t005_module, "load_anchor", spy_load_anchor)
    anchor = t005_module._load_t005_anchor(_PID)
    ...
    monkeypatch.setattr(t005_module, "load_anchor", mismatched_load_anchor)
    with pytest.raises(AnchorError, match="load_anchor selected"):
        t005_module._load_limit_anchor(
            "PDG2025:T005:ATLAS2022:t_cg",
            process_id=_PID,
        )
...
def test_evaluate_without_input_degrades_gracefully():
    ...
    assert result.sm_prediction == pytest.approx(4.6e-12)
    assert result.budget == pytest.approx(3.7e-4)
...
def test_sm_limit_and_manual_width_recomputation():
    ...
    point = point_builder.make_point(quark_mass_basis_couplings=couplings)
    result = constraint.evaluate(point)
    manual = _manual_t_to_c_gluon_proxy_br(
        couplings=couplings,
