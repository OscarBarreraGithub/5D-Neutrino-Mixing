...
@pytest.mark.parametrize(
    ("point", "expected_pass"),
    [
        (rare_charm_point_with_wilsons(lepton="mu", c10_np=1.0), True),
        (rare_charm_point_with_wilsons(lepton="mu", c10_np=2.0e4), False),
    ],
)
def test_safe_point_passes_and_large_np_point_fails(
    ...
    direct = evaluate_d0_to_ll(
        core_dilepton_wilsons_from_rs_coeff(rs_coeff(point, lepton="mu")),
        lepton="mu",
    )
...
def test_optional_kk_ew_mass_extra_is_diagnostic_only_no_second_suppression():
    default_point = rare_charm_point_with_wilsons(lepton="mu", c10_np=123.0)
