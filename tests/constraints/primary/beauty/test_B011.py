"""Production tests for B011 (inclusive Bbar -> X_s gamma)."""
...
_SIDECAR = _REPO_ROOT / "flavor_catalog" / "processes" / "beauty" / "B011.yaml"
...
def _manual_c7_prediction(
    *,
    ...
    branching_fraction = sm_branching_fraction * ratio_to_sm
    return {
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(Bbar -> X_s gamma)"
...
def test_anchor_matches_yaml_and_loud_fail_probe():
    ...
    sm = pdg["sm_prediction_current"]
    validation = pdg["sm_prediction_pdg_review_quote"]
    combined_sigma = math.sqrt(exp["uncertainty"] ** 2 + sm["uncertainty"] ** 2)
    central_residual = abs(exp["value"] - sm["value"])
    ...
    assert constraint.anchor.validation_standard_model.source_url == (
...
def test_sm_limit_branching_fraction_validates_misiak_anchor():
    constraint = fcc.get(_PID)
    ...
    assert result.predicted == pytest.approx(3.40e-4)
...
def test_constraint_matches_shared_bsgamma_module_recomputation():
    constraint = fcc.get(_PID)
    left = 0.03e-1 + 0.02e-1j
    right = 1.0e-2 - 0.3e-2j
    couplings = _bs_couplings(left=left, right=right)
...
@pytest.mark.parametrize(
    ...
    [
        (_bs_couplings(left=0.0j, right=1.0e-2), True),
...
def test_optional_kk_ew_mass_extra_changes_matching_scale():
    couplings = _bs_couplings(left=0.0j, right=1.2e-1)
    ...
    assert abs(heavy_result.diagnostics["c7_np_matching"]) == pytest.approx(
