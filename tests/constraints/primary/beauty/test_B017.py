"""Production tests for B017 (B -> K(*) ell ell / R_K context)."""
...
from flavor_catalog_constraints.primary.beauty import B017 as b017_module
...
_SIDECAR = _REPO_ROOT / "flavor_catalog" / "processes" / "beauty" / "B017.yaml"
...
def test_registration_metadata():
    ...
    assert constraint.observable == "R_K central-q2 in B -> K(*) ell+ ell- context"


def test_anchor_matches_yaml_and_budget_band():
    constraint = fcc.get(_PID)
    obs = _yaml_pdg_block()["observables"]
    inclusive = _find_entry(obs, "name", "BR(B -> X_s ell+ ell-)")
    charged = _find_entry(obs, "name", "BR(B+ -> K+ ell+ ell-)")
    kstar = _find_entry(obs, "name", "BR(B0 -> K*(892)0 ell+ ell-)")
    active = _find_entry(obs, "name", "R_K central-q2")
    rkstar = _find_entry(obs, "name", "R_K* central-q2")

    active_upper = math.sqrt(0.042**2 + 0.022**2)
    active_lower = math.sqrt(0.041**2 + 0.022**2)
    active_sigma = 0.5 * (active_upper + active_lower)
    central = abs(float(active["value"]) - 1.0)
    proxy = b017_module.RARE_B_DILEPTON_EXCLUSIVE_BK_PROXY_THEORY_UNCERTAINTY_FRACTION

    assert constraint.anchor.inclusive_total.value == pytest.approx(
        float(inclusive["value"]) * 1.0e-6
    )
    assert constraint.anchor.charged_total.value == pytest.approx(
        float(charged["value"]) * 1.0e-7
    )
    assert constraint.anchor.kstar_total.value == pytest.approx(
        float(kstar["value"]) * 1.0e-7
    )
    assert constraint.anchor.rk_central.value == pytest.approx(float(active["value"]))
    assert constraint.anchor.rk_central.uncertainty_upper == pytest.approx(
        active_upper
    )
    assert constraint.anchor.rk_central.uncertainty_lower == pytest.approx(
        active_lower
    )
    ...
    assert constraint.anchor.rkstar_central.value == pytest.approx(
        float(rkstar["value"])
    )
    ...
    assert constraint.anchor.budget == pytest.approx(central + active_sigma + proxy)
    assert constraint.anchor.budget == pytest.approx(0.3979713194305231)
    assert "NEEDS-HUMAN-PHYSICS" in constraint.anchor.budget_band.construction

    with pytest.raises(AnchorError):
        b017_module._load_observable(
            process_id=_PID,
            observable_name="no such B017 observable",
        )
...
def test_observable_anchors_route_through_scaffold_load_anchor_and_fail_loudly(
    ...
    original_load_anchor = b017_module.anchor_scaffold.load_anchor

    def spy_load_anchor(*args, **kwargs):
        calls.append(tuple(kwargs["candidates"]))
        return original_load_anchor(*args, **kwargs)

    monkeypatch.setattr(b017_module.anchor_scaffold, "load_anchor", spy_load_anchor)
    anchor = b017_module._load_b017_anchor(_PID)
    ...
    assert calls == [
        ...
        ("pdg_or_equivalent.observables[6]",),
    ]
...
def test_evaluate_without_input_degrades_gracefully():
    ...
    assert result.diagnostics["inclusive_scope"].startswith("context_anchor_only")
...
def test_sm_limit_rk_proxy_matches_independent_integral():
    constraint = fcc.get(_PID)
...
def test_np_prediction_matches_underlying_core_recomputation():
    ...
    direct_mu = evaluate_b_to_k_mumu(
        core_wilsons_from_rs_coeff(
...
def test_evaluate_runs_end_to_end_with_real_finite_fields_and_complex_diagnostics():
    ...
    for key in (
        ...
        "charged_total_branching_fraction",
    ...
    assert result.diagnostics["inclusive_scope"].startswith("context_anchor_only")
    assert "K* angular" in result.diagnostics["kstar_angular_scope"]
...
def test_safe_point_passes_and_large_np_point_fails(
    ...
    if expected_pass:
        ...
        assert result.predicted == pytest.approx(1.0)
        assert result.diagnostics["rk_proxy_muon_ratio_to_sm"] == pytest.approx(
            result.diagnostics["rk_proxy_electron_ratio_to_sm"]
        )
    else:
        assert result.ratio > 1.0
        assert result.predicted != pytest.approx(1.0)
