"""Per-constraint contract tests for E002 (EDM placeholder)."""
...
def test_registered():
    assert "E002" in ConstraintRegistry.all()


def test_anchor_matches_yaml():
    c = ConstraintRegistry.get("E002")
    assert c.anchor.observable
    assert c.anchor.source


def test_sm_point_passes(sm_point):
    r = ConstraintRegistry.get("E002").evaluate(sm_point)
...
def test_excluded_point_does_not_veto(excluded_point):
    ...
    r = ConstraintRegistry.get("E002").evaluate(excluded_point)
    assert r.passes is True
