"""Per-constraint contract tests for E008 (EDM placeholder)."""
...
def test_registered():
    assert "E008" in ConstraintRegistry.all()


def test_anchor_matches_yaml():
    c = ConstraintRegistry.get("E008")
    assert c.anchor.observable
    assert c.anchor.source


def test_sm_point_passes(sm_point):
    r = ConstraintRegistry.get("E008").evaluate(sm_point)
...
def test_excluded_point_does_not_veto(excluded_point):
    ...
    r = ConstraintRegistry.get("E008").evaluate(excluded_point)
    assert r.passes is True
