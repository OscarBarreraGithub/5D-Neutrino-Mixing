"""Per-constraint contract tests for E001 (EDM placeholder)."""
...
def test_registered():
    assert "E001" in ConstraintRegistry.all()


def test_anchor_matches_yaml():
    c = ConstraintRegistry.get("E001")
    assert c.anchor.observable
    assert c.anchor.source


def test_sm_point_passes(sm_point):
    r = ConstraintRegistry.get("E001").evaluate(sm_point)
...
def test_excluded_point_does_not_veto(excluded_point):
    ...
    r = ConstraintRegistry.get("E001").evaluate(excluded_point)
    assert r.passes is True
