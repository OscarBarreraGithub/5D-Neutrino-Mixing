"""Per-constraint contract tests for E006 (EDM placeholder)."""
...
def test_registered():
    assert "E006" in ConstraintRegistry.all()


def test_anchor_matches_yaml():
    c = ConstraintRegistry.get("E006")
    assert c.anchor.observable
    assert c.anchor.source


def test_sm_point_passes(sm_point):
    r = ConstraintRegistry.get("E006").evaluate(sm_point)
...
def test_excluded_point_does_not_veto(excluded_point):
    ...
    r = ConstraintRegistry.get("E006").evaluate(excluded_point)
    assert r.passes is True
