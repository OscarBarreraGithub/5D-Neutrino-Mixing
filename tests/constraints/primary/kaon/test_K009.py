"""Per-constraint contract tests for K009 (``K_L -> pi0 mu+ mu-``).
...
2. ``test_anchor_matches_yaml`` - typed view matches the yaml on the
   fields K009 actually reads (flat ``pdg_or_equivalent`` upper-limit block).
3. ``test_sm_point_passes`` - currently passes because evaluation is
   explicitly deferred pending Delta S = 1 rare-kaon muon-mode machinery.
...
YAML_PATH = REPO_ROOT / "flavor_catalog" / "processes" / "kaon" / "K009.yaml"


def test_registered():
    c = ConstraintRegistry.get("K009")
    assert c.process_id == "K009"
    assert c.observable == "BR(K_L -> pi0 mu+ mu-)"
...
def test_anchor_matches_yaml():
    """The typed anchor pins the PDG K_L -> pi0 mu+ mu- upper limit."""
    c = ConstraintRegistry.get("K009")
    raw = yaml.safe_load(open(YAML_PATH))["pdg_or_equivalent"]

    assert raw["value"] == "<3.8e-10"
    ...
    assert c.anchor.upper_limit == 3.8e-10
...
def test_sm_point_passes(sm_point):
    ...
    r = ConstraintRegistry.get("K009").evaluate(sm_point)
    assert r.process_id == "K009"
    assert r.passes is True
    assert r.predicted is None
    assert r.experimental == 3.8e-10
    assert r.severity == Severity.INFO
    assert r.diagnostics["deferred"] is True
    assert r.diagnostics["upper_limit"] == 3.8e-10
    assert r.diagnostics["sm_branching_ratio_order"] == 1.5e-11
    assert (
    ...
    assert "indirect-CP" in r.diagnostics["evaluation_deferred"]
    assert "photon" in r.diagnostics["evaluation_deferred"]


def test_excluded_point_fails(excluded_point):
    """Even a K001/K002-excluded point passes because K009 evaluation is deferred."""
    r = ConstraintRegistry.get("K009").evaluate(excluded_point)
    assert r.passes is True
    assert r.predicted is None
...
def test_adapter_result_is_marked_deferred():
    """Adapter exposes the explicit deferred bit requested for K009."""
    result = evaluate_klong_pi0mumu_placeholder()
