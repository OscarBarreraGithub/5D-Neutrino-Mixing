...
def test_sm_limit_branching_fraction_matches_independent_recomputation():
    ...
    direct_klong = evaluate_klong_mumu_short_distance(couplings)
...
def test_evaluate_runs_end_to_end_with_real_finite_fields_and_complex_diagnostics():
    ...
    assert result.diagnostics["uses_k006_muon_sd_wilson_proxy"] is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_sd_couplings(left=2.0e-1j), False),
    ],
)
def test_safe_point_passes_and_large_np_point_fails(
    ...
    result = constraint.evaluate(point)
    direct_klong = evaluate_klong_mumu_short_distance(couplings)
    wilsons = compute_rare_kaon_dilepton_wilsons(couplings, inputs=constraint.sm_inputs)
...
def test_real_only_np_coupling_leaves_ks_imaginary_sd_near_sm():
    constraint = fcc.get(_PID)
    couplings = _sd_couplings(left=2.0e-1, right=0.0j)
    point = point_builder.build_from_quark_couplings(couplings)
    result = constraint.evaluate(point)
