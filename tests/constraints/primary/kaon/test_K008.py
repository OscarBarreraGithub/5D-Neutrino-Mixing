"""Per-constraint contract tests for K008 (``K_L -> pi0 e+ e-``).
...
2. ``test_anchor_matches_yaml`` - typed view matches the yaml on the
   fields K008 actually reads (flat ``pdg_or_equivalent`` upper-limit block).
...
YAML_PATH = REPO_ROOT / "flavor_catalog" / "processes" / "kaon" / "K008.yaml"


def test_registered():
    c = ConstraintRegistry.get("K008")
    assert c.process_id == "K008"
    assert c.observable == "BR(K_L -> pi0 e+ e-)"
...
def test_anchor_matches_yaml():
    """The typed anchor pins the PDG K_L -> pi0 e+ e- upper limit."""
    c = ConstraintRegistry.get("K008")
    raw = yaml.safe_load(open(YAML_PATH))["pdg_or_equivalent"]

    assert raw["value"] == "<2.8e-10"
    ...
    assert c.anchor.upper_limit == 2.8e-10
...
def test_sm_point_passes(sm_point):
    ...
    r = ConstraintRegistry.get("K008").evaluate(sm_point)
    assert r.process_id == "K008"
    assert r.passes is True
    assert r.predicted is None
    assert r.experimental == 2.8e-10
    assert r.severity == Severity.INFO
    assert r.diagnostics["deferred"] is True
    assert r.diagnostics["upper_limit"] == 2.8e-10
    assert r.diagnostics["sm_branching_ratio_order"] == 3.0e-11
    assert (
    ...
    assert "gamma gamma" in r.diagnostics["evaluation_deferred"]


def test_excluded_point_fails(excluded_point):
    """Even a K001/K002-excluded point passes because K008 evaluation is deferred."""
    r = ConstraintRegistry.get("K008").evaluate(excluded_point)
    assert r.passes is True
    assert r.predicted is None
...
def test_adapter_result_is_marked_deferred():
    """Adapter exposes the explicit deferred bit requested for K008."""
