"""Production tests for B007 (B_s/B0 -> e+ e-)."""
...
from flavor_catalog_constraints.secondary.beauty import B007 as b007_module
...
def _value_row(row_id: str):
    for row in _yaml_doc()["pdg_or_equivalent"]["values"]:
        if row["id"] == row_id:
            return row
    raise AssertionError(f"no B007 pdg_or_equivalent.values row with id={row_id!r}")


def _sm_row(source_key: str, observable: str):
    groups = _yaml_doc()["auxiliary_theory_inputs"]["standard_model_predictions"]
    for group in groups:
        if group["source_key"] != source_key:
            continue
        for row in group["predictions"]:
            if row["observable"] == observable:
                return group, row
    raise AssertionError(
        f"no B007 SM row with source_key={source_key!r}, observable={observable!r}"
    )
...
def _manual_sm_ee(inputs, transition: str) -> float:
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(B_s -> e+ e-), BR(B0 -> e+ e-)"


def test_anchor_matches_yaml_and_budget_rows():
    constraint = fcc.get(_PID)
    bs_limit = _value_row("pdg2026_bs_ee_90cl")
    bd_limit = _value_row("pdg2026_bd_ee_90cl")
    bs_group, bs_sm = _sm_row("BobethEtAl2013_BqllSM", "SM BR(B_s -> e+e-)")
    bd_group, bd_sm = _sm_row("BobethEtAl2013_BqllSM", "SM BR(B_d -> e+e-)")
    ...
    assert constraint.anchor.bs_standard_model.source_url == bs_group["source_url"]
...
def test_b007_anchors_route_through_scaffold_load_anchor(monkeypatch):
    calls: list[tuple[str, ...]] = []
    original_load_anchor = b007_module.load_anchor
    ...
    constraint = fcc.get(_PID)
    monkeypatch.setattr(b007_module, "load_anchor", spy_load_anchor)
    anchor = b007_module._load_b007_anchor(
        _PID,
    ...
    assert calls == [
        ("values[0]",),
        ("values[1]",),
        ("auxiliary_theory_inputs.standard_model_predictions[1].predictions[0]",),
        ("auxiliary_theory_inputs.standard_model_predictions[1].predictions[1]",),
    ]
...
def test_loud_fail_for_mismatched_limit_anchor_block(monkeypatch):
    ...
    monkeypatch.setattr(b007_module, "load_anchor", wrong_block_load_anchor)

    with pytest.raises(AnchorError, match=r"expected 'values\[0\]' for B007 anchor"):
        b007_module._limit_anchor_from_value_row(_PID, row_id="pdg2026_bs_ee_90cl")
...
def test_loud_fail_for_missing_limit_row_and_limit_below_sm():
    ...
    with pytest.raises(AnchorError, match="expected exactly one"):
        b007_module._value_row_by_id(_PID, "not_a_real_row")
    ...
    with pytest.raises(AnchorError, match="limit must exceed the SM anchor"):
        b007_module._build_channel_budget(
            process_id=_PID,
            channel_key="b_s",
...
def test_sm_limit_branching_fractions_match_independent_formula_and_anchors():
    constraint = fcc.get(_PID)
    point = point_builder.build_from_quark_couplings(_bq_couplings())
    result = constraint.evaluate(point)
    channels = result.diagnostics["channels"]
    manual_bs = _manual_sm_ee(constraint.sm_inputs, "b_s")
    manual_bd = _manual_sm_ee(constraint.sm_inputs, "b_d")
    ...
    assert manual_bs == pytest.approx(8.540154879838497e-14)
    assert manual_bd == pytest.approx(2.450169556007537e-15)
...
@pytest.mark.parametrize(
    ...
    [
        (_bq_couplings(bs_left=10.0, bd_left=10.0), True),
        (_bq_couplings(bs_left=100.0, bd_left=100.0), False),
    ],
)
...
def test_optional_kk_ew_mass_extra_changes_matching_scale():
    couplings = _bq_couplings(bs_left=40.0, bd_left=40.0)
    ...
    for channel in ("b_s", "b_d"):
        ...
        assert abs(ew_channel["c10_leptonic_np"]) == pytest.approx(
            abs(default_channel["c10_leptonic_np"]) / 4.0
        )
