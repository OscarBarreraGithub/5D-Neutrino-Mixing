"""Production tests for K021 (K_L -> pi0 e+- mu-+ LFV)."""
...
from flavor_catalog_constraints.secondary.kaon import K021 as k021_module
...
def _value_entry(value_id: str):
    for entry in _yaml()["pdg_or_equivalent"]["values"]:
        if entry["value_id"] == value_id:
            return entry
    raise AssertionError(f"no K021 value entry for {value_id!r}")
...
def _independent_klong_pi0_emu_branching(
    quark_couplings: QuarkMassBasisCouplings,
    lepton_proxy: dict[str, complex | float | str],
    inputs,
    *,
...
def test_registration_metadata():
    ...
    assert constraint.observable == "BR(K_L -> pi0 e+- mu-+) LFV"


def test_anchor_matches_yaml_and_routes_through_scaffold_load_anchor(monkeypatch):
    calls: list[tuple[str, ...]] = []
    original_load_anchor = k021_module.load_anchor

    def spy_load_anchor(*args, **kwargs):
        calls.append(tuple(kwargs["candidates"]))
        return original_load_anchor(*args, **kwargs)

    monkeypatch.setattr(k021_module, "load_anchor", spy_load_anchor)
    anchor = k021_module._load_k021_anchor(_PID)
    current = _value_entry(k021_module._CURRENT_LIMIT_VALUE_ID)
    ktev = _value_entry(k021_module._KTEV_VALUE_ID)
    ...
    assert anchor.ktev_limit.value == pytest.approx(float(ktev["value"]))
    assert anchor.ktev_limit.source_url == ktev["source_url"]
    assert anchor.value == pytest.approx(7.6e-11)
    assert anchor.budget == pytest.approx(anchor.value)


def test_k021_anchor_loud_fail_probe(monkeypatch):
    broken = _yaml()
    first = dict(broken["pdg_or_equivalent"]["values"][0])
    first["limit_type"] = "measurement"
    broken["pdg_or_equivalent"]["values"][0] = first

    def fake_load_full_yaml(*_args, **_kwargs):
        return broken

    monkeypatch.setattr(k021_module, "load_full_yaml", fake_load_full_yaml)
    with pytest.raises(k021_module.AnchorError):
        k021_module._load_k021_anchor(_PID)
...
def test_lfv_branching_fraction_matches_independent_recomputation():
    ...
    independent = _independent_klong_pi0_emu_branching(
        quark_couplings,
        lepton_proxy,
    ...
    assert result.diagnostics["neutral_mode"] is True
    assert result.diagnostics["klong_mass_gev"] == pytest.approx(0.497611)
    assert result.diagnostics["pi0_mass_gev"] == pytest.approx(0.1349768)
    assert result.diagnostics["klong_lifetime_s"] == pytest.approx(5.116e-8)
