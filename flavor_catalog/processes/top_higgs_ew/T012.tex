...
\subsection*{Process}
This entry covers the charm-quark Z-pole observables \(R_c^0\),
\(A_{\rm FB}^{0,c}\), and \(A_c\).  Plan v1 lists \(R_c\) as T012 and the
charm asymmetries as T013; the assignment asks for the combined
\(Z\to c\bar c\) pole-observable package under process id T012.
...
accessed on 2026-05-16.  The measured values are
\[
...
conversion.  The LEP/SLC final-combination snapshot in
...
heavy-flavour fit.  Since arXiv:0804.1954, the RS literature has made the
model dependence of ordinary \(Zf\bar f\) couplings more explicit; the
Casagrande--Goertz--Haisch--Neubert--Pfoh setup treats the standard \(Z\)
couplings, flavor-changing \(Z\) effects, and electroweak precision constraints
in a unified warped-flavor framework.  On the Standard-Model side, the
fermionic electroweak two-loop calculation of Z partial widths and branching
ratios was completed in arXiv:1401.2447.  Prospective FCC-ee work, including
arXiv:2107.00616, now revisits how \(R_c\) and \(A_{\rm FB}^{c}\) could be
improved with modern heavy-flavour tagging, but those studies are not current
experimental inputs.
