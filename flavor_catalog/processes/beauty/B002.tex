...
\subsection*{Process}
The process is the mixing-induced CP asymmetry in
\(B_d^0\to J/\psi K_S^0\), conventionally reported as
\(S_{\psi K_S}\simeq \sin 2\beta\) or \(\sin 2\phi_1\).  It is the
beauty-sector benchmark CP phase of the unitarity triangle and is dominated by
interference between \(B_d-\bar B_d\) mixing and the \(b\to c\bar c s\)
tree amplitude.

\subsection*{PDG / equivalent value(s)}
The source-of-truth numerical block is
\texttt{pdg\_or\_equivalent} in \texttt{B002.yaml}.  HFLAV Summer 2025 gives
...
\]
using \(6\,{\rm fb}^{-1}\), and Belle II measured
\[
  S=0.724\pm0.035_{\rm stat}\pm0.009_{\rm syst}
\]
with \(362\,{\rm fb}^{-1}\).  These inputs moved the HFLAV average to the
percent-level regime.  On the theory side, the main development is not a new
operator but a sharper accounting of doubly Cabibbo-suppressed penguin
pollution; Frings--Nierste--Wiebusch quote
...
PARTIAL.  No implementation named \texttt{S\_psi}, \texttt{J/psi K\_S},
...
input, \texttt{quarkConstraints/deltaf2.py:631}--\texttt{:638} define its
hadronic and \(\Delta m_d\) inputs, and
