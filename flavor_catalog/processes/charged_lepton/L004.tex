...
\]
The PDG line identifies the document as BERTL 06, technique SPEC, comment
SINDRUM II.  The local minimal text snapshot is
...
replaced the SINDRUM II bound in the PDG listing.  The major change is the
experimental program around lighter targets: the Mu2e Technical Design Report
states an aluminum conversion search aiming for sensitivity approximately four
orders of magnitude beyond the previous world-best conversion limits, and the
COMET Phase-I TDR quotes an aluminum sensitivity goal \(3.1\times10^{-15}\)
with an expected \(90\%\) upper limit \(7\times10^{-15}\).  Thus the gold
limit remains the published benchmark, while future comparisons will need
