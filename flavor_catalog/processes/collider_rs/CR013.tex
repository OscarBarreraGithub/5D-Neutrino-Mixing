...
coupling \(k/\bar M_{\rm Pl}\).  In the collider-RS catalog this is the
direct neutral-resonance search for spin-2 KK gravitons and nearby scalar
partners in the high-mass \(\gamma\gamma\) spectrum.
...
\texttt{solvers/bessel.py:5} and \texttt{solvers/bessel.py:6} solve KK masses
for gauge bosons and fermions in RS backgrounds,
\texttt{scanParams/scan.py:399} and \texttt{scanParams/scan.py:429} compute
and record \(M_{\rm KK}\), and \texttt{quarkConstraints/deltaf2.py:1},
\texttt{quarkConstraints/deltaf2.py:459}, and
\texttt{quarkConstraints/deltaf2.py:557} implement the low-energy
\(\Delta F=2\) KK-gluon-inspired exclusion slice.  None of those
