...
The Astro 5.x static site that surfaces the 103-entry flavor-physics
catalog (`../`; 95 PRIMARY + 8 SECONDARY) as a browsable, filterable web
artifact for collaborators. It renders one page per process, per-family
indices, a Pagefind-powered browse/filter view, a "How this catalog was built"
methodology page, and an **Explore** tab (`/explore`) that visualizes the
quark-only scan floors.

The Explore tab is fed by a separate content file
`src/content/scan_explorer.json`, generated by `scripts/build_scan_explorer.py`
from the raw scan outputs (not by the catalog ingest below). See
[`docs/STATE_OF_PROJECT.md`](../../docs/STATE_OF_PROJECT.md) (Web Tool section)
and [`docs/FLOOR_SUMMARY.md`](../../docs/FLOOR_SUMMARY.md) for what it shows.
...
- `dist/entries/<id>/index.html` -- 103 entry pages
...
- `dist/explore/index.html` -- Scan Explorer (quark-only floor envelopes)
