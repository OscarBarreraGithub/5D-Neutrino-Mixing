...
The 50\%-acceptance crossing is at $\Mkk = 16.54$~TeV (perturbative
$g_s$) or $47.26$~TeV ($\gs = 3$); the 95\%-acceptance crossing is at
$44.50$~TeV / $127.13$~TeV. These are the headline numbers of this analysis,
quoted from the post-audit 8M-draw RUNA reference scan
(\S\ref{sec:robustness}). They are conditional on the BGS~2020 central
$\eK$ SM prediction and on the leading-log Wilson-RG convention audited in
Appendix~\ref{app:input-audits}.
...
BGS-budget band, not an NLO Wilson-RG uncertainty; the latter remains an
order-$10$--$30\%$ theory systematic.
...
$0.5\%$ under the central BGS + LO-only conventions.
...
\begin{figure}[h]
  ...
  ($c \to c - 0.05$) returned zero PDG-passing draws over 1.6M attempts
...
both cases the resulting 1.6M-draw scan returned exactly zero
PDG-passing draws.
...
\begin{figure}[h]
  ...
  Csaki--Falkowski--Weiler 0804.1954. Under the corrected BGS + LO-only
  convention, our factor-of-three 95\%-acceptance crossing
  ($\sim\!127$~TeV) is well above the historical CFW band; the comparison
  at the CFW gate (factor-of-1.5, $|Y|\ge 0.5$) is not made because that
  gate returned zero PDG-passing draws over our 4M total samples (see
  text below).}
...
honest comparison is narrower: our post-audit $95\%$-acceptance bound at
the factor-of-three gate, $\Mkk\!\sim\!127$~TeV in the $\gs = 3$
convention, is not a CFW reproduction and should not be compared as one
number-for-number. It uses modern BGS/FLAG inputs and the audited
leading-log LR running, while CFW used different historical inputs and a
different inverse $(c,\tilde m)$ scan design. We therefore treat the CFW
overlay only as historical context. A full numerical reproduction of the
CFW band at their gate is left as a follow-up that would require the
inverse sampler we have not built.
...
necessarily qualitative. The post-audit $95\%$ bound at $\gs=3$
($\sim 127$~TeV) is above the historical $21$--$33$~TeV CFW band under
our BGS + LO-only conventions, so the overlay is now context rather than
band-overlap
