...
\begin{figure}[h]
  ...
  imposes CFW's 30\% relative mass/CKM/$J$ gate.  The vertical markers are
  the CFW abstract values: $21$~TeV for generic RS anarchy and $33$~TeV for
  the pseudo-Goldstone Higgs case.}
...
\]
This is $11\%$ above CFW's $21$~TeV generic-RS marker in the common
$\gs=3$ plotting convention, so we regard the convention-matched
comparison as qualitatively consistent at the 20\% level.  The statement
is deliberately narrower than a literal rerun: CFW's published $21$~TeV
number is tied to a boundary-term color-coupling convention often described
as $\gs\sim6$, while their no-UV-boundary-term variant lowers the
same bound to $10.5$~TeV.  If one multiplies our matched curve by a further
$6/3$, the p50 becomes $46.7$~TeV; we therefore quote the agreement only
after putting both curves into the common $\gs=3$ convention used in
Fig.~\ref{fig:robust-cfw}.
...
quantitatively stable only after matching historical conventions.  Under
our live BGS + LO-only conventions the p50 value is $47.26$~TeV, but
switching to the CFW-era $\epsilon_K$ budget and 30\% relative mass/CKM/$J$
gate gives $23.37$~TeV in the common $\gs=3$ convention, within $11\%$ of
CFW's $21$~TeV generic-RS marker
