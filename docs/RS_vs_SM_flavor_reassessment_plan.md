...
> pattern. But run forward with anarchy, that same input forces M_KK >~ 10-30 TeV
> from epsilon_K and >~ 18-20 TeV *irreducibly* from oblique T, while every
> would-be flavor "prediction" (phi_s, C_Bs, D0, S_psiphi) has collapsed onto the
> SM point, and the S-T ellipse has **recentered on the SM**. Custodialization
> rescues the EW sector but not epsilon_K.
...
2. **The one surviving virtue: geometric hierarchy + RS-GIM as a single input.**
   ...
   localization that gives the 5-decade hierarchy sets the FCNC coefficients.
...
5. **Have the predictions collapsed? The SM-overlap test.** The operational
   ...
   was dropped from the production scan, so it is not in the quark M12 parquet) — it
   is flagged as a separate lepton-proxy follow-up (sec 9 future work), not a joint
...
7. **The cures and what they cost.** Alignment / 5D-MFV / shining drive flavor floors
   ...
   T/Zbb, not epsilon_K. *Fig 7:* S1 (anarchic) vs S2 (aligned) endpoints; custodial-
   vs-minimal epsilon_K overlay.
...
- **C4 SM-overlap scoreboard (Fig 5)** — post-process the M12 parquet (M12 ->
  ...
  experimental resolutions (phi_s +-16 mrad; ATLAS/CMS/LHCb). **No new scan** (see D1).
...
- **C5 S1-vs-S2 alignment endpoints** — existing parquets (`anarchic_bauer_S1/S2`).
  S1 median epsilon_K crossing ~19 TeV vs S2 ~1.8 TeV on the stored grids.
...
scan (cut; leptons cannot rescue the kaon wall); full PC dual translation.
...
paper, not this reassessment. Optional single intermediate point only if a referee
challenges the endpoints as too binary.
...
- **Floor-definition ambiguity:** quote typical (median) AND existence floors always;
  state the (A)/(B) verdict against the typical floor.
...
1. **STEP 0** (no compute): freeze the evaluative standard ("better on flavor" =
   viable + predictive FCNC suppression *without* imposed flavor symmetry).
