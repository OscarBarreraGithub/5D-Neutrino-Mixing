...
$\alpha_Q\,\mathbf{I}$ term entirely. In its place, an ad hoc nonlinear map
(next section) clamps the eigenvalues into a bounded window after
...
The eigenvalues are non-negative, but they are not directly the bulk mass
parameters $c$ that enter the $f$-factor formula. We apply an ad hoc bounded
sigmoid to all three sectors ($Q$, $u$, $d$) with the same parameters:
\begin{equation}
...
This map is not equivalent to the standard NMFV parametrization. It preserves
the eigenvectors of $C_X$ but distorts the eigenvalue splittings in a
nonlinear way that has no counterpart in the spurion expansion. It also uses
the same map for all three sectors, removing sector-dependent universal offsets
(i.e.\ independent $\alpha_Q$, $\alpha_u$, $\alpha_d$) and
sign/cancellation freedom in the $O(1)$ coefficients. In the standard
parametrization, $\alpha_Q$ provides a flavor-universal baseline and the
eigenvalues of $C_Q$ are directly the bulk masses with no remapping needed.
...
$\alpha_u$, $\alpha_d$ for the RH sectors), the sigmoid would be replaced by a
direct identification $c_{X,i} = \text{eigenvalue}_i(C_X)$, with independent
...
captures localization dependence but is not the full 5D overlap integral).
...
$\text{ratio} \le 1$. These are deliberately conservative acceptance rules
(generous budgets, NP-only with no SM interference), so the resulting bounds
should be read as a weak ``best-case NP-only envelope,'' not as a sharp
physical exclusion in a global-fit sense.
