...
\fcolorbox{red!70!black}{red!5}{%
  \parbox{0.92\linewidth}{%
    \textbf{SUPERSEDED.} This Apr 16 walkthrough is preserved for historical
    reference. For the current state of the pipeline see
    \texttt{docs/quark\_scan\_consolidation\_report.tex} (May 2026 audit +
    cleanup + C02c measured band).%
  }%
}
\end{center}
...
running with $\alpha_s(M_Z) = 0.1180$ and continuous threshold matching.
...
The matrix elements used by the scan are
\begin{equation}
...
$\langle Q_1\rangle = \tfrac{8}{3}\,f_P^2\,m_P^2\,B_K$ and divide by
...
For kaons, the post-audit inputs are $B_K^{(1)} = 0.5503$,
$B_K^{(4)} = 0.903$, and $B_K^{(5)} = 0.691$.
...
$|2.228\times10^{-3} - 2.161\times10^{-3}| = 6.7\times10^{-5}$.
The absolute value discards the sign of the NP contribution, so there is no
constructive/destructive interference with SM.
