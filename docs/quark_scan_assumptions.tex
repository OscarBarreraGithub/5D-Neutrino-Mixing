...
generally dense.  All off-diagonal flavor structure in the left-handed quark
sector, and therefore all tree-level FCNCs, come from the combination of the
two left rotations $U_{\mathrm{left}}^u$ and $U_{\mathrm{left}}^d$
through~$C_Q$.
...
Our scan omits $\alpha_Q\,\mathbf{I}$.  Its role is instead played by the
...
The sigmoid is needed because we omit $\alpha_Q$ from $C_Q$: without a
flavor-universal piece, the eigenvalues of $C_Q$ can range from 0 to
$\infty$, and there is no guarantee they land in a physically sensible bulk
mass window.  The sigmoid clamps them into $[0.30,\,0.72]$ by construction.
In the standard parametrization with explicit $(\alpha_Q,\,\beta_Q)$, no such
map is needed: the eigenvalues of $C_Q$ \emph{are} the bulk masses directly,
and $\alpha_Q$ provides the baseline that keeps them in range.

\textcolor{red}{\textbf{To do:} With the explicit
$(\alpha_Q,\,\beta_Q,\,\gamma_Q)$ parametrization (see the $C_Q$ TODO above),
the BulkMassMap would be replaced by a direct identification
$c_{Q,i} = \text{eigenvalue}_i(C_Q)$, with $\alpha_Q$ providing the
flavor-universal offset. The sigmoid is an artifact of the current
parametrization and should be removed in future scans.}
