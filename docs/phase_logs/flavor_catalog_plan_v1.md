# Flavor Catalog Planner v1

## Changelog (v0 → v1)
- BLOCKER 1 (Section D): added Deliverables and Success criteria blocks for WA, CA, DA, Opus.
- BLOCKER 2 (Section G): added hard iteration caps with mandatory sidecar status transitions on cap hit.
- WARNING (Section B): clarified `status` field semantics (chose status_history list with timestamped transitions).
...
% v1 revision: Planning-round reference updated from v0 to v1.
- Store generated figures only under `flavor_catalog/figures/by_process/<process_id>/generated/`, with source code or notebooks under `source/`. No figures are required in v1 unless a process has a canonical plot that materially clarifies a constraint.
...
% v1 revision: Scalar `status` was replaced by timestamped `status_history`; the latest status is the final transition's `to` value.
...
status_history:
  - from: null
    to: "DRAFT"
    at: "2026-05-16T00:00:00-04:00"
    agent_id: "PKA-K-001"
    reason: "initial process skeleton"
checker_passed_at: null
opus_approved_at: null
last_updated_at: "2026-05-16T00:00:00-04:00"
version: "0.1.0"
source_shas:
...
% v1 revision: Legal status transitions are explicit so `OPUS-APPROVED` cannot be mistaken for a simultaneous scalar state with `CHECKER-DONE`.

Status semantics and legal transition graph:

- The sidecar's current status is the `to` value of the final `status_history` entry. There is no separate scalar `status` field.
- Every transition entry must include `from`, `to`, `at`, `agent_id`, and `reason`. `catalog_index.yaml` should mirror the same history or a validated summary derived from it.
- Normal approval path: `DRAFT -> PKA-DONE -> WRITER-DONE -> CHECKER-DONE -> OPUS-APPROVED`.
- Checker failure loop: `WRITER-DONE -> WRITER-REWORK -> WRITER-DONE`; the CA worklog must list the issues that forced rework.
- Opus return loop: `CHECKER-DONE -> RETURN-TO-WA -> WRITER-DONE -> CHECKER-DONE -> OPUS-APPROVED`.
- PKA cap triage path: `DRAFT` or `PKA-DONE -> WA-TRIAGE -> WRITER-DONE`, or `BLOCKED-PI` if the source/value problem needs PI judgment.
- Arbitration path: `WRITER-REWORK` or `RETURN-TO-WA -> OPUS-ARBITRATION -> RETURN-TO-WA`, `OPUS-APPROVED`, or `BLOCKED-PI`.
- Discovery out-of-scope path: proposal-only entries may end at `DEFERRED-SCOPE`; in-scope additions enter the normal path at `DRAFT`.
- `checker_passed_at` is set only when a `CHECKER-DONE` transition is appended. `opus_approved_at` is set only when an `OPUS-APPROVED` transition is appended. `last_updated_at` changes on every sidecar edit.
- `OPUS-APPROVED` is valid only if the same `status_history` contains a prior `CHECKER-DONE` transition after the last WA change.
...
% v1 revision: WA now has explicit deliverables and success criteria matching the PKA operational standard.

Deliverables:

- Updated `flavor_catalog/processes/<family>/<process_id>.tex` for every process in the batch.
- Updated `flavor_catalog/processes/<family>/<process_id>.yaml` for every process in the batch, including a `status_history` transition to `WRITER-DONE` or a documented `WA-TRIAGE` outcome.
- Proposed entries or a patch for `flavor_catalog/references/catalog.bib`, with any unresolved keys listed in the writer worklog.
- `flavor_catalog/worklogs/writer/<batch_id>.md`, including process list, source decisions, unresolved `\textbf{CHECK}` items, bibliography changes, and version/status transitions.

Success criteria:

- Every process in the batch has all required TeX sections present and normalized to the process template.
- Every `\textbf{CHECK}` marker is mirrored in the YAML `open_issues` list and the writer worklog.
- Every accepted PKA source appears in the sidecar `source_shas` or in a documented rejection note.
- `status_history` ends at `WRITER-DONE` for processes ready for CA, or at `WA-TRIAGE`/`BLOCKED-PI` with a signoff-log cap entry if Section G requires escalation.
- No agent may be both WA and CA on the same batch in the same round; the orchestrator must enforce this before CA dispatch.
...
% v1 revision: CA now has explicit deliverables, status-output contract, and role-separation success criteria.

Deliverables:

- `flavor_catalog/worklogs/checker/<batch_id>.md`, containing PASS/FAIL, an actionable issue list, and a verified source/value table with process id, canonical value or limit, uncertainty/CL, source, access date, local snapshot path, and sha256.
- Updated `flavor_catalog/processes/<family>/<process_id>.yaml` sidecars with `status_history` transitions: `WRITER-DONE -> CHECKER-DONE` on pass, or `WRITER-DONE -> WRITER-REWORK` on fail.
- For failures, a compact rework checklist in the checker worklog that the next WA cycle can close item by item.

Success criteria:

- The checker checklist is fully closed for every process in the batch; unchecked items are failures, not caveats.
- The verified source/value table agrees with local snapshots and `source_shas`.
- Every PASS appends `CHECKER-DONE`, sets `checker_passed_at`, and records the CA agent id in the sidecar.
- Every FAIL appends `WRITER-REWORK` with issue references and leaves `checker_passed_at` null or unchanged from the last valid checker pass before the new WA edit.
- The CA did not serve as WA for the same batch in the same round; if the orchestrator detects overlap, the CA result is invalid and must be rerun by a different agent.
...
% v1 revision: DA now has concrete output paths and an additions/merge/no-change proposal artifact.

Deliverables:

- `flavor_catalog/worklogs/discovery/round_<NNN>_<scope>.md`, including search queries, source families checked, additions considered, duplicates found, and no-change rationale.
- `flavor_catalog/worklogs/discovery/round_<NNN>_<scope>_proposal.patch`, containing one of: additions to `catalog_index.yaml` plus process stubs, merge/rename proposals for duplicate entries, or an explicit no-change proposal.
- For proposed additions, a table with process id, family, source, one-line rationale, likely canonical value source, initial code-coverage guess, and whether PKA is required.
...
- Every proposed addition has a legal initial status path: in-scope additions enter `DRAFT`; rejected or deferred proposals are logged as `DEFERRED-SCOPE` with a signoff entry.
- The proposal patch applies cleanly or is explicitly marked as a review-only patch with the conflicting paths named.

% v1 revision: DA repetition is now bounded by Section G's hard max of four total discovery rounds unless PI explicitly expands scope.

Run at least two DA rounds. Converge only after two consecutive discovery rounds find at most one genuinely new process each and every existing process has CA pass plus Opus approval, subject to the hard cap and escalation rules in Section G.
...
% v1 revision: Opus now has explicit signoff artifacts, outcomes, and success criteria.

Deliverables:

- `flavor_catalog/signoff/round_<NNN>_index.md`, summarizing all processes reviewed in the round, commit SHA, input batch ids, verdicts, cap hits, and unresolved PI questions.
- `flavor_catalog/signoff/by_process/<process_id>.md`, recording per-process version, source manifest sha, checker worklog path, Opus verdict, sidecar transition, and caveats.
- Sidecar `status_history` updates for the Opus outcome: APPROVE maps to `CHECKER-DONE -> OPUS-APPROVED`; RETURN-TO-WA maps to `CHECKER-DONE -> RETURN-TO-WA`; ESCALATE-TO-PI maps to `CHECKER-DONE -> BLOCKED-PI` unless the PI has already scoped the process out as `DEFERRED-SCOPE`.

Success criteria:

- Every signoff document has an explicit verdict: APPROVE, RETURN-TO-WA, or ESCALATE-TO-PI.
- Every APPROVE has a commit SHA, source manifest sha, prior `CHECKER-DONE` transition, `opus_approved_at`, and final sidecar transition to `OPUS-APPROVED`.
- Every RETURN-TO-WA names the exact WA-owned fixes and consumes the single Opus corrective loop allowed by Section G.
- Every ESCALATE-TO-PI appends `BLOCKED-PI` or `DEFERRED-SCOPE` and records the PI question in the round index so no process remains silently open.
...
6. Claude assigns different CAs to those batches. A CA cannot check its own WA batch, and no agent can be both WA and CA on the same batch in the same round.
7. Mismatches go back to the WA with the CA worklog attached. Iterate WA->CA until CA passes, bounded by Section G's WA/CA cap.
...
% v1 revision: DA repetition is bounded by Section G's four-round cap; open-ended repetition from v0 is replaced by deterministic escalation.

9. DA round 2 starts after all initial and DA-1 additions have CA pass. Repeat only while the convergence rule is not met and the total DA round count remains below the Section G hard cap, unless the PI explicitly expands scope.
...
% v1 revision: Convergence now refers to `status_history`, not simultaneous scalar states.

- Every in-scope process sidecar has a `CHECKER-DONE` transition after the last WA edit and ends with `OPUS-APPROVED`. Processes ending at `BLOCKED-PI` or `DEFERRED-SCOPE` are not silently open; they require an explicit signoff entry and PI/scope disposition before final catalog acceptance.
- `catalog_master.tex` compiles.
- `catalog_index.yaml` has no latest status of `DRAFT`, `PKA-DONE`, `WA-TRIAGE`, `WRITER-DONE`, `WRITER-REWORK`, `CHECKER-DONE`, `RETURN-TO-WA`, `OPUS-ARBITRATION`, or stale source snapshot statuses.
...
% v1 revision: v0 expected-round guidance is now paired with hard ceilings and required cap-hit status transitions.

Hard iteration caps:

- PKA source-fix micro-iterations: max 2 per process before WA triage. After the second failed source/value fix, the orchestrator appends `WA-TRIAGE` if a WA can resolve the issue from sources, or `BLOCKED-PI` if the issue requires PI judgment. The cap hit must be recorded in `flavor_catalog/signoff/round_<NNN>_index.md` and `flavor_catalog/signoff/by_process/<process_id>.md` before further work.
- WA/CA full cycles per batch: max 3, where one cycle is WA delivery followed by CA pass/fail. After the third failed CA review for any process in the batch, the orchestrator appends `OPUS-ARBITRATION` and sends the batch to Opus; if Opus cannot resolve it within the corrective loop cap, the sidecar ends at `BLOCKED-PI`.
- DA discovery rounds: max 4 total across all DA scopes, unless the PI explicitly expands scope in writing. At the fourth round, remaining additions are either accepted into `DRAFT` with assigned PKA ownership, merged into existing entries, or marked `DEFERRED-SCOPE`; each disposition must appear in the round signoff index.
- Opus corrective loops: max 1 per process after a RETURN-TO-WA verdict. If the corrected process still cannot be approved, Opus appends `BLOCKED-PI` or `DEFERRED-SCOPE` and records the exact PI decision needed.

Cap-hit logging requirements:

- Every cap hit creates or updates both `flavor_catalog/signoff/round_<NNN>_index.md` and `flavor_catalog/signoff/by_process/<process_id>.md`.
- Every cap-hit sidecar update appends a `status_history` transition to `WA-TRIAGE`, `OPUS-ARBITRATION`, `BLOCKED-PI`, or `DEFERRED-SCOPE`; no process may remain with a stale latest status after a cap is reached.
- Cap-hit log entries record process id, batch id, cap name, attempted iteration count, current version, latest commit SHA, responsible next owner, and the minimum condition for reopening.
...
% v1 revision: Status acceptance now refers to status_history rather than a scalar status field.

- `status_history` includes `OPUS-APPROVED` for every accepted in-scope process; any `BLOCKED-PI` or `DEFERRED-SCOPE` entry has an explicit PI/scope disposition before final acceptance.
