"""Tau-electron radiative charged-LFV adapter.

This adapter reuses the L001 lepton-dipole machinery for ``mu -> e gamma`` and
pins it to ``tau -> e gamma`` by permuting the charged-lepton rows before
...
is the canonical ``(e, tau)`` dipole entry of the original matrix.

NEEDS-HUMAN-PHYSICS: a rigorous RS prediction for ``tau -> e gamma`` requires
...
__all__ = [
    ...
    "TauToEGammaProxyInput",
    "TauToEGammaBranchingResult",
    "tau_to_e_gamma_proxy_input",
    "tau_to_e_gamma_from_lepton_input",
]

TAU_TO_E_GAMMA_PROXY_V1 = (
    "NEEDS-HUMAN-PHYSICS: tau->e gamma is evaluated as a documented proxy "
    "using the same Perez-Randall NDA lepton-dipole normalization as L001. "
    "The adapter maps the L001 core's (e,mu) matrix element to the tau->e "
    "(e,tau) element by a charged-lepton row permutation; full tau radiative "
    "RS loop matching is not implemented."
)

_ROW_PERMUTATION_E_TAU_MU = (0, 2, 1)
_FINAL_FLAVOR = "e"
_INITIAL_FLAVOR = "tau"
_FINAL_INDEX = 0
_INITIAL_INDEX = 2
...
class TauToEGammaProxyInput:
    """Explicit proxy inputs for the tau->e radiative LFV dipole.
    ...
    m_kk_gev: float
    source: str = "caller-supplied tau->e gamma dipole proxy"


@dataclass(frozen=True)
class TauToEGammaBranchingResult:
    """Branching-fraction view of the tau->e dipole-bound proxy."""
    ...
    off_diagonal_13: complex
    off_diagonal_31: complex
...
def tau_to_e_gamma_proxy_input(
    y_n_bar: Any,
    pmns: Any,
    m_kk_gev: float,
    *,
    ...
    """Build a shape-checked proxy input for ``tau_to_e_gamma_from_lepton_input``."""
...
def tau_to_e_gamma_from_lepton_input(
    lepton_input: Any,
    *,
    br_limit: float,
    prefactor_br: float,
    ...
    """Evaluate proxy ``BR(tau -> e gamma)`` from a YukawaResult or proxy input."""

    _assert_tau_to_e_flavors(lepton_input)
    limit = _positive_finite(br_limit, name="br_limit")
    raw_prefactor = _positive_finite(prefactor_br, name="prefactor_br")
    # PDG B(tau->e nu nubar) converts the reused muon-normalized NDA width to a tau BR.
    prefactor = float(raw_prefactor * TAU_TO_E_NUNU_BRANCHING_FRACTION)
    ...
    if _is_yukawa_result_like(lepton_input):
        ...
        core = _check_mu_to_e_gamma(
            _YukawaResultRowView(
                Y_N_matrix=y_matrix[list(_ROW_PERMUTATION_E_TAU_MU), :],
                params=params,
            ),
            C=c_lfv,
    ...
    core = _check_mu_to_e_gamma_raw(
        y_n_bar,
        pmns[list(_ROW_PERMUTATION_E_TAU_MU), :],
        M_KK=m_kk,
        C=c_lfv,
        reference_scale=reference_scale,
    )
    return _result_from_core(
        ...
        reference_scale_gev=reference_scale,
        m_kk_gev=m_kk,
        input_kind="TauToEGammaProxyInput",
        used_proxy=True,
        extra_diagnostics={
...
def _result_from_core(
    ...
    *,
    ...
    off_13 = complex(product_matrix[_FINAL_INDEX, _INITIAL_INDEX])
    off_31 = complex(product_matrix[_INITIAL_INDEX, _FINAL_INDEX])
    diagnostics = {
        ...
        "branching_formula": (
            "BR_NP = muon_normalized_prefactor_br * B(tau->e nu nubar) * "
            "|(Y_N_bar Y_N_bar^dagger)_{e tau}|^2 "
            "* (reference_scale_gev / M_KK)^4"
        ),
        "muon_normalized_prefactor_br": float(raw_prefactor_br),
        "tau_leptonic_branching_fraction": float(TAU_TO_E_NUNU_BRANCHING_FRACTION),
        ...
        "row_permutation_for_mu_to_e_core": _ROW_PERMUTATION_E_TAU_MU,
        "core_off_diagonal_12_after_permutation": complex(core["off_diagonal_12"]),
        **dict(extra_diagnostics),
    }
    return TauToEGammaBranchingResult(
        ...
        off_diagonal_13=off_13,
        off_diagonal_31=off_31,
        product_matrix=_matrix_tuple(product_matrix),
        row_permutation=_ROW_PERMUTATION_E_TAU_MU,
...
def _proxy_from_mapping(mapping: Mapping[str, Any]) -> TauToEGammaProxyInput:
    return tau_to_e_gamma_proxy_input(
        _mapping_value(mapping, "y_n_bar", "Y_N_bar"),
        _mapping_value(mapping, "pmns", "PMNS", "pmns_matrix"),
        _mapping_value(mapping, *_M_KK_KEYS),
        source=str(mapping.get("source", "mapping tau->e gamma dipole proxy")),
    )


def _coerce_proxy(value: Any) -> TauToEGammaProxyInput:
    if isinstance(value, TauToEGammaProxyInput):
        return value
    ...
    raise TypeError(
        ...
        "TauToEGammaProxyInput, or a mapping with y_n_bar, pmns, and m_kk_gev"
    )


def _assert_tau_to_e_flavors(value: Any) -> None:
    ...
    if initial is not None and _canonical_flavor(initial) != _INITIAL_FLAVOR:
        raise ValueError(
            "tau->e gamma adapter is pinned to initial_flavor='tau'; "
            f"got {initial!r}"
        )
    if final is not None and _canonical_flavor(final) != _FINAL_FLAVOR:
        raise ValueError(
            "tau->e gamma adapter is pinned to final_flavor='e'; "
            f"got {final!r}"
        )
