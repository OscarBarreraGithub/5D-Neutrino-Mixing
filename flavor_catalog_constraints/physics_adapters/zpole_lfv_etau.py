"""Adapter for ``Z -> e tau`` LFV constraints.

This module reuses the shared :mod:`quarkConstraints.zpole_lfv`
off-diagonal-coupling branching-fraction machinery, but supplies an e-tau
proxy extractor.  Constraint modules import this adapter only.

NEEDS-HUMAN-PHYSICS: ``ParameterPoint`` does not carry a rigorous
off-diagonal lepton ``Z e tau`` effective coupling.  The proxy path maps
caller-supplied lepton overlap spurions onto
``delta g_etau ~ (m_Z/M_KK)^2 overlap_etau`` and is explicitly diagnostic-only
...
__all__ = [
    ...
    "ZPoleSMInputs",
    "ZPoleLFVBranchingResult",
    "ZPoleLFVETauProxyInput",
    "ZPoleLFVETauCouplingProxy",
    "zpole_lfv_etau_proxy_input",
    "zpole_lfv_etau_sm_total_width_weight",
    "zpole_lfv_etau_effective_coupling_limit",
    "zpole_lfv_etau_coupling_proxy",
    "zpole_lfv_etau_branching_fraction_with_proxy",
]

ZPOLE_LFV_ETAU_PROXY_V1 = (
    "NEEDS-HUMAN-PHYSICS: off-diagonal lepton Z e-tau couplings are not "
    "available on ParameterPoint; caller-supplied lepton overlap spurions are "
    "mapped to delta g_etau = proxy_strength * (m_Z/M_KK)^2 * overlap_etau, "
...
class ZPoleLFVETauProxyInput:
    """Explicit lepton-overlap proxy for the LFV ``Z e tau`` coupling."""

    left_etau_overlap: complex
    right_etau_overlap: complex
    m_kk_gev: float
    source: str = "caller-supplied lepton neutral-current e-tau proxy"


@dataclass(frozen=True)
class ZPoleLFVETauCouplingProxy:
    """Documented proxy mapping e-tau overlap spurions to ``Z e tau`` shifts."""
    ...
    right_etau_overlap: complex
    scale_factor: float
    delta_g_left_etau: complex
    delta_g_right_etau: complex
    source: str
    diagnostics: Mapping[str, Any]


def zpole_lfv_etau_proxy_input(
    left_etau_overlap: complex,
    right_etau_overlap: complex,
    m_kk_gev: float,
    *,
    ...
    """Build a shape-checked explicit proxy input for T016-like constraints."""

    return ZPoleLFVETauProxyInput(
        left_etau_overlap=complex(left_etau_overlap),
        right_etau_overlap=complex(right_etau_overlap),
        m_kk_gev=_positive_float("m_kk_gev", m_kk_gev),
        source=str(source),
    )


def zpole_lfv_etau_sm_total_width_weight(
...
def zpole_lfv_etau_effective_coupling_limit(
    br_limit: float,
    *,
    inputs: ZPoleSMInputs | None = None,
) -> float:
...
def zpole_lfv_etau_coupling_proxy(
    source: object,
    *,
    m_kk_gev: float | None = None,
    inputs: ZPoleSMInputs | None = None,
) -> ZPoleLFVETauCouplingProxy:
    """Return the documented lepton-overlap proxy for ``Z e tau`` couplings."""
    ...
    left = complex(p.proxy_strength * scale * proxy_input.left_etau_overlap)
    right = complex(p.proxy_strength * scale * proxy_input.right_etau_overlap)
    return ZPoleLFVETauCouplingProxy(
        model_label=ZPOLE_LFV_MODEL_V1,
        matching_assumption=ZPOLE_LFV_ETAU_PROXY_V1,
        M_KK=resolved_m_kk,
        matching_scale=resolved_m_kk,
        left_etau_overlap=complex(proxy_input.left_etau_overlap),
        right_etau_overlap=complex(proxy_input.right_etau_overlap),
        scale_factor=scale,
        delta_g_left_etau=left,
        delta_g_right_etau=right,
...
def zpole_lfv_etau_branching_fraction_with_proxy(
    source: object,
    *,
    br_limit: float | None = None,
    m_kk_gev: float | None = None,
    inputs: ZPoleSMInputs | None = None,
) -> tuple[ZPoleLFVBranchingResult, ZPoleLFVETauCouplingProxy]:
    """Evaluate ``BR(Z -> e tau)`` with the documented e-tau proxy."""

    p = default_sm_inputs() if inputs is None else inputs
    proxy = zpole_lfv_etau_coupling_proxy(source, m_kk_gev=m_kk_gev, inputs=p)
    result = _branching_from_couplings(
        delta_g_left=proxy.delta_g_left_etau,
        delta_g_right=proxy.delta_g_right_etau,
        initial_flavor="e",
        final_flavor="tau",
        br_limit=br_limit,
        inputs=p,
...
def _coerce_proxy_input(
    ...
    *,
    ...
    if isinstance(value, ZPoleLFVETauProxyInput):
        return value
...
def _proxy_from_mapping(
    ...
    *,
    ...
    source = str(mapping.get("source", "mapping lepton neutral-current e-tau proxy"))
    left, right = _overlaps_from_mapping(mapping)
    return ZPoleLFVETauProxyInput(
        left_etau_overlap=left,
        right_etau_overlap=right,
        m_kk_gev=_resolve_m_kk_from_mapping(mapping, m_kk_gev),
        source=source,
    )


def _proxy_from_object(value: Any, *, m_kk_gev: float | None) -> ZPoleLFVETauProxyInput:
    left = _first_present_attr(value, ("left_etau_overlap", "left_etau"))
    right = _first_present_attr(value, ("right_etau_overlap", "right_etau"))
    if left is None and right is None:
        ...
        if left_matrix is None and right_matrix is None:
            raise TypeError(
                "lepton LFV e-tau proxy must provide left/right e-tau overlap "
                "entries or charged-lepton overlap matrices"
            )
        left = 0.0j if left_matrix is None else _matrix_offdiag(left_matrix, "left", 0, 2)
        right = 0.0j if right_matrix is None else _matrix_offdiag(right_matrix, "right", 0, 2)
    return ZPoleLFVETauProxyInput(
        left_etau_overlap=complex(0.0j if left is None else left),
        right_etau_overlap=complex(0.0j if right is None else right),
        m_kk_gev=_resolve_m_kk_from_object(value, m_kk_gev),
        source=str(getattr(value, "source", "object lepton neutral-current e-tau proxy")),
    )


def _overlaps_from_mapping(mapping: Mapping[str, Any]) -> tuple[complex, complex]:
    left = _first_present_key(mapping, ("left_etau_overlap", "left_etau"))
    right = _first_present_key(mapping, ("right_etau_overlap", "right_etau"))
    ...
    if left_matrix is None and right_matrix is None:
        raise KeyError(
            "mapping must provide left/right e-tau overlap entries or "
            "charged-lepton overlap matrices"
        )
    left_value = 0.0j if left_matrix is None else _matrix_offdiag(left_matrix, "left", 0, 2)
    right_value = 0.0j if right_matrix is None else _matrix_offdiag(right_matrix, "right", 0, 2)
...
def _resolve_m_kk_from_object(value: Any, override: float | None) -> float:
    ...
    raise AttributeError("lepton LFV e-tau proxy object must provide m_kk_gev or M_KK")
