"""T014 - FCNC Z down-sector (SECONDARY placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "FCNC Z down-sector")) if isinstance(inner, dict) else "FCNC Z down-sector"
...
class Constraint:
    ...
    observable = "FCNC Z down-sector"

    def __init__(self) -> None:
        raw = load_raw(self.process_id, family="top_higgs_ew", tier="secondary")
        self.anchor = _build_anchor(raw)
        self.references = (self.anchor.source,)

    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("Z FCNC")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: T014 SECONDARY placeholder.",
