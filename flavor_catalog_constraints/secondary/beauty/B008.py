"""B008 - rare tauonic leptonic neutral-B decays.

Physics
-------
``BR(B_s -> tau+ tau-)`` and ``BR(B0 -> tau+ tau-)`` reuse the shared
``quarkConstraints.rare_b_dilepton`` Buras ``b -> q l l`` machinery through
the tau-mode adapter
``flavor_catalog_constraints.physics_adapters.rare_b_tauonic``.  The same
C10-dominant leptonic formula is used with the charged-lepton mass set to
``m_tau``, making this the largest SM branching fraction among the three
charged-lepton modes.

Severity
--------
HARD.  The SM predictions are far below the PDG 2026 95%-CL one-mode-at-a-time
limits in ``B008.yaml``, so this constraint treats each limit as an essentially
pure-NP branching-fraction budget.  The reported HARD ratio is the largest
channel saturation.

Catalog sidecar
---------------
``flavor_catalog/processes/secondary/beauty/B008.yaml`` is the source of
...
from flavor_catalog_constraints.physics_adapters.rare_b_tauonic import (
...
_BS_LIMIT_ROW_ID = "pdg2026_bs_tautau_95cl"
_BD_LIMIT_ROW_ID = "pdg2026_bd_tautau_95cl"
_SM_SOURCE_KEY = "BobethEtAl2014_BqTauTauSM"
_BS_SM_OBSERVABLE = "SM BR(B_s -> tau+ tau-)"
_BD_SM_OBSERVABLE = "SM BR(B_d -> tau+ tau-)"
_BUDGET_SOURCE = (
    "flavor_catalog/processes/secondary/beauty/B008.yaml "
    "pdg2026_bs_tautau_95cl + pdg2026_bd_tautau_95cl, with "
    "BobethEtAl2014_BqTauTauSM SM references"
)
...
class TauonicLimitAnchor:
    """Upper-limit anchor loaded from one B008 ``pdg_or_equivalent.values`` row."""
...
@dataclass(frozen=True)
class TauonicSMAnchor:
    """SM prediction anchor loaded from B008 auxiliary theory inputs."""
...
@dataclass(frozen=True)
class TauonicChannelBudget:
    """Pure-NP upper-limit budget for one tauonic neutral-B channel."""
...
@dataclass(frozen=True)
class B008Anchor:
    """Typed B008 anchor bundle for both ``B_s`` and ``B0`` tau modes."""

    bs_limit: TauonicLimitAnchor
    bd_limit: TauonicLimitAnchor
    bs_standard_model: TauonicSMAnchor
    bd_standard_model: TauonicSMAnchor
    bs_budget: TauonicChannelBudget
    bd_budget: TauonicChannelBudget
...
def _required_float(value: Any, *, process_id: str, field_name: str) -> float:
    try:
        ...
        raise AnchorError(
            f"{process_id}: B008 anchor field {field_name!r}={value!r} is not numeric"
        ) from exc
    if not math.isfinite(number):
        raise AnchorError(
            f"{process_id}: B008 anchor field {field_name!r}={value!r} is not finite"
        )
    return number
...
def _positive_float(value: Any, *, process_id: str, field_name: str) -> float:
    ...
    if number <= 0.0:
        raise AnchorError(f"{process_id}: B008 field {field_name!r} must be positive")
    return number
...
def _pdg_block(process_id: str) -> Mapping[str, Any]:
    ...
    if not isinstance(block, Mapping):
        raise AnchorError(
            f"{process_id}: expected mapping-shaped 'pdg_or_equivalent' for B008"
        )
    return block
    ...
    *,
    ...
    if anchor.block_key != block_key:
        raise AnchorError(
            ...
            f"expected {block_key!r} for B008 anchor"
        )
    return anchor
...
def _limit_anchor_from_value_row(process_id: str, *, row_id: str) -> TauonicLimitAnchor:
    ...
    if anchor.units != _EXPECTED_UNITS:
        raise AnchorError(
            f"{process_id}: B008 limit {row_id!r} must use units "
            f"{_EXPECTED_UNITS!r}, got {anchor.units!r}"
        )
    if anchor.value <= 0.0 or not math.isfinite(anchor.value):
        raise AnchorError(f"{process_id}: B008 limit {row_id!r} must be positive")
    ...
    if row.get("limit_type") != "upper":
        raise AnchorError(f"{process_id}: B008 limit {row_id!r} must be upper-type")
    return TauonicLimitAnchor(
        anchor=anchor,
        row_id=row_id,
...
def _sm_auxiliary_row(
    process_id: str,
    *,
    source_key: str,
    observable: str,
) -> tuple[int, Mapping[str, Any]]:
    rows = _full_yaml(process_id).get("auxiliary_theory_inputs")
    if not isinstance(rows, list) or not rows:
        raise AnchorError(f"{process_id}: auxiliary_theory_inputs must be a list")
    matches: list[tuple[int, Mapping[str, Any]]] = []
    for index, row in enumerate(rows):
        if not isinstance(row, Mapping):
            raise AnchorError(f"{process_id}: auxiliary_theory_inputs[{index}] is not a mapping")
        if row.get("source_key") == source_key and row.get("observable") == observable:
            matches.append((index, row))
    if len(matches) != 1:
        raise AnchorError(
            f"{process_id}: expected exactly one auxiliary_theory_inputs row "
            f"with source_key={source_key!r}, observable={observable!r}; "
            f"found {len(matches)}"
        )
    return matches[0]


def _sm_anchor_from_auxiliary_input(
    process_id: str,
    *,
    source_key: str,
    observable: str,
) -> TauonicSMAnchor:
    index, row = _sm_auxiliary_row(
        process_id,
        source_key=source_key,
        observable=observable,
    )
    block_key = f"auxiliary_theory_inputs[{index}]"
    anchor = _anchor_from_virtual_block(
        process_id,
        block_key=block_key,
        row=row,
    ...
    if anchor.units != _EXPECTED_UNITS:
        raise AnchorError(
            f"{process_id}: B008 SM row {observable!r} must use units "
            f"{_EXPECTED_UNITS!r}, got {anchor.units!r}"
        )
    if anchor.value <= 0.0 or not math.isfinite(anchor.value):
        raise AnchorError(f"{process_id}: B008 SM row {observable!r} must be positive")
    if anchor.uncertainty is None or anchor.uncertainty <= 0.0:
        raise AnchorError(
            f"{process_id}: B008 SM row {observable!r} requires a positive uncertainty"
        )
    return TauonicSMAnchor(anchor=anchor, source_key=source_key, raw_row=row)
...
def _build_channel_budget(
    *,
    process_id: str,
    channel_key: str,
    limit: TauonicLimitAnchor,
    standard_model: TauonicSMAnchor,
    formula_sm: float,
) -> TauonicChannelBudget:
    if limit.value <= standard_model.value:
        raise AnchorError(
            f"{process_id}: B008 {channel_key} limit must exceed the SM anchor"
        )
    hard_budget = float(limit.value)
    return TauonicChannelBudget(
        ...
        construction=(
            "Pure-NP upper-limit budget: because the B008 SM branching "
            "fraction is far below the experimental 95%-CL limit, compare "
            "max(0, BR_total - BR_SM(anchor)) with BR_limit and also require "
            "BR_total <= BR_limit. Limits are one-mode-at-a-time."
        ),
    )


def _load_b008_anchor(
    process_id: str,
    *,
    formula_sm_bs: float,
    formula_sm_bd: float,
    ...
    bs_sm = _sm_anchor_from_auxiliary_input(
        process_id,
        source_key=_SM_SOURCE_KEY,
        observable=_BS_SM_OBSERVABLE,
    )
    bd_sm = _sm_anchor_from_auxiliary_input(
        process_id,
        source_key=_SM_SOURCE_KEY,
...
@register
class Constraint:
    """Catalogued ``B_s/B0 -> tau+ tau-`` branching-ratio limit constraint."""

    process_id = "B008"
    severity = Severity.HARD
    observable = "BR(B_s -> tau+ tau-), BR(B0 -> tau+ tau-)"

    def __init__(self) -> None:
        self.sm_inputs = rare_b_tauonic_default_sm_inputs()
        self.bs_sm_result = rare_b_tauonic_sm_branching_fraction(
            "b_s",
            self.sm_inputs,
        )
        self.bd_sm_result = rare_b_tauonic_sm_branching_fraction(
            "b_d",
            self.sm_inputs,
        )
        self.anchor = _load_b008_anchor(
            self.process_id,
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        if rs_wilsons is None:
            return ConstraintResult(
                ...
                notes=(
                    f"extra {_REQUIRED_EXTRA!r} absent; B_s/B0 -> tau+tau- "
        ...
        return ConstraintResult(
            ...
            diagnostics={
                ...
                "tau_mass_gev": float(TAU_MASS_GEV),
