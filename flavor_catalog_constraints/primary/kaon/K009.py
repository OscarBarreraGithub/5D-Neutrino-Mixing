"""K009 - Long-lived neutral-kaon rare muon mode ``K_L -> pi0 mu+ mu-``.
...
requires ``s -> d mu+ mu-`` Wilson matching, CKM inputs, SM--NP
interference conventions, the K_S -> pi0 mu+ mu- indirect-CP input, and
...
Catalog sidecar
---------------
``flavor_catalog/processes/kaon/K009.yaml`` (schema-flex anchor:
flat ``pdg_or_equivalent`` block with value ``"<3.8e-10"``).

Physics core
------------
No Delta S = 1 rare-kaon muon-mode implementation exists yet. The
placeholder reaches
...
_SM_BRANCHING_RATIO_ORDER = 1.5e-11


@dataclass(frozen=True)
class _Anchor:
    """Typed view over ``K009.yaml``'s flat ``pdg_or_equivalent`` block."""
...
def _parse_upper_limit(raw_value: str) -> float:
    """Parse catalog upper-limit strings like ``<3.8e-10``."""
...
def _build_anchor(raw: dict[str, Any]) -> _Anchor:
    """K009 uses a flat ``pdg_or_equivalent`` upper-limit anchor."""
...
@register_constraint
class Constraint:
    """Catalogued K_L -> pi0 mu+ mu- constraint (process_id ``K009``)."""

    process_id = "K009"
    severity = Severity.INFO
    observable = "BR(K_L -> pi0 mu+ mu-)"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        return ConstraintResult(
            ...
            notes=(
                "Evaluation deferred: K009 is a Delta S=1 rare kaon "
                "muon mode with short-distance + indirect-CP-violation "
            ...
            diagnostics={
                ...
                "sm_branching_ratio_order_source": (
                    "Isidori-Smith-Unterdorfer 2004 arXiv hep-ph/0404127"
                ),
                "rate_components": (
                    "short-distance",
                ...
                "required_machinery": (
                    "s -> d mu+ mu- Wilson matching, CKM elements, SM-NP "
                    "interference conventions, K_S -> pi0 mu+ mu- indirect-CP "
