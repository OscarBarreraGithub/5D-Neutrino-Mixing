"""E004 — Neutron electric dipole moment.
...
Catalog sidecar
---------------
``flavor_catalog/processes/edm_neutrino/E004.yaml`` (schema-flex anchor:
...
class _Anchor:
    """Typed view over the ``E004.yaml`` anchor block (schema-flex)."""
...
def _build_anchor(raw) -> _Anchor:
    ...
    return _Anchor(
        raw=raw,
        observable=str(inner.get("observable", "Neutron EDM |d_n|")),
...
@register_constraint
class Constraint:
    """Catalogued Neutron electric dipole moment constraint (process_id ``E004``)."""

    process_id = "E004"
    severity = Severity.INFO
    observable = "Neutron EDM |d_n|"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_hadronic_edm_placeholder("neutron")
        return ConstraintResult(
            ...
            notes=(
                "Evaluation deferred: E004 is an EDM observable requiring "
