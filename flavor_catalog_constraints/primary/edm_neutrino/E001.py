"""E001 — Electron electric dipole moment.
...
Catalog sidecar
---------------
``flavor_catalog/processes/edm_neutrino/E001.yaml`` (schema-flex anchor:
``pdg_or_equivalent.canonical_limit`` block).
"""
...
class _Anchor:
    """Typed view over the ``E001.yaml`` anchor block (schema-flex)."""
...
def _build_anchor(raw) -> _Anchor:
    ...
    block = raw.get("canonical_limit", raw)
    ...
    return _Anchor(
        raw=raw,
        observable=str(inner.get("observable", "Electron EDM |d_e|")),
...
@register_constraint
class Constraint:
    """Catalogued Electron electric dipole moment constraint (process_id ``E001``)."""

    process_id = "E001"
    severity = Severity.INFO
    observable = "Electron EDM |d_e|"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_lepton_edm_placeholder("electron")
        return ConstraintResult(
            ...
            notes=(
                "Evaluation deferred: E001 is an EDM observable requiring "
