"""E002 - muon electric dipole moment ``|d_mu|``.

Physics
-------
The Standard Model muon EDM is negligible at the precision of the current
direct bound, so E002 is a pure-new-physics HARD constraint:
...
``flavor_catalog_constraints.physics_adapters.edm``:
    ...
    d_mu [e cm] = c_mu^CP-odd [GeV^-1] * hbar*c [GeV cm].
...
available on ``ParameterPoint``. If the caller supplies an explicit muon EDM
proxy coefficient through ``lepton_mass_basis_couplings``, the result is
evaluated but flagged ``NEEDS-HUMAN-PHYSICS`` in diagnostics.

Catalog sidecar
---------------
``flavor_catalog/processes/edm_neutrino/E002.yaml`` is the source of truth for
the PDG/Bennett direct muon-EDM upper limit. Numeric limit values are loaded
...
_SM_MUON_EDM_E_CM = 0.0
_UNEVALUATED_REASON = (
    "no muon EDM dipole prediction available "
...
class MuonEDMAnchor:
    """Typed E002 anchor: current direct upper limit plus limit metadata."""
    ...
    def value(self) -> float:
        """Upper limit on ``|d_mu|`` in e cm."""
        return self.experimental.value

    @property
    def budget(self) -> float:
        """HARD veto budget for the pure-NP muon EDM."""
        return self.experimental.value
...
def _load_e002_anchor(process_id: str) -> MuonEDMAnchor:
    ...
    if experimental.block_key != _CURRENT_LIMIT_BLOCK:
        raise AnchorError(
            ...
            f"expected {_CURRENT_LIMIT_BLOCK!r} for the current direct muon EDM limit"
        )
    if experimental.value <= 0.0:
        raise AnchorError(f"{process_id}: muon EDM limit must be positive")
...
@register
class Constraint:
    """Catalogued direct muon EDM bound."""
    ...
    def __init__(self) -> None:
        self.anchor = _load_e002_anchor(self.process_id)
        self.sm_value = _SM_MUON_EDM_E_CM
    ...
    def _unevaluated_result(
        ...
        *,
        ...
        merged_diagnostics = {
            ...
            "passes_semantics": "non-vetoing only; no muon EDM prediction was evaluated",
            "budget_source": self.anchor.source_url,
            "experimental_block": self.anchor.experimental.block_key,
            "sm_edm_policy": (
                "SM muon EDM is negligible at catalog precision; "
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        try:
            result = charged_lepton_edm_from_lepton_input(
                lepton_input,
                lepton="mu",
        ...
        diagnostics.update(
            {
                ...
                "sm_prediction_policy": (
                    "SM d_mu is negligible compared with the current direct bound; "
        ...
        return ConstraintResult(
            ...
            notes=(
                "Pure-NP |d_mu| bound using d_mu[e cm] = c_CPodd[GeV^-1] "
                "* hbar*c. HARD budget is the E002.yaml canonical direct limit. "
