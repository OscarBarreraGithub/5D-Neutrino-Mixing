"""L004 - coherent mu->e conversion in gold.

Physics
-------
The Standard Model charged-LFV conversion rate is negligible for catalog
purposes, so L004 is a pure-new-physics bound,

    CR(mu Au -> e Au) = omega_conv / Gamma_capture.

The prediction uses the same reusable Kitano-Koike-Okada overlap-integral
machinery as L003, reached only through
``flavor_catalog_constraints.physics_adapters.mu_e_conversion``.  This file
selects the gold target nuclear inputs and the observed SINDRUM II Au limit.
...
are flagged in diagnostics.  Unknown dipole-contact phases use the conservative
destructive-interference envelope for the HARD verdict and report the full
interval.

Catalog sidecar
---------------
``flavor_catalog/processes/charged_lepton/L004.yaml`` is the source of truth
for the observed SINDRUM II gold conversion bound.  L001.yaml supplies the
...
_BUDGET_POLICY = (
    "Observed SINDRUM II Au 90% C.L. upper limit from L004.yaml; HARD veto "
    "uses the pure-NP CR(mu Au -> e Au) prediction normalized to the gold "
    "capture rate."
)
...
@dataclass(frozen=True)
class L004Anchor:
    """Typed L004 inputs: SINDRUM II Au limit plus L001 dipole normalization."""
...
def _load_l004_anchor(process_id: str) -> L004Anchor:
    ...
    if experimental.value <= 0.0:
        raise AnchorError(f"{process_id}: Au conversion limit must be positive")
    ...
    if target != "Au":
        raise AnchorError(f"{process_id}: primary limit target must be 'Au', got {target!r}")
    ...
    if dipole_limit.value <= 0.0 or dipole_prefactor.value <= 0.0:
        raise AnchorError("L004: L001 dipole normalization inputs must be positive")
...
@register
class Constraint:
    """Catalogued coherent mu->e conversion constraint for gold."""

    process_id = "L004"
    severity = Severity.HARD
    observable = "CR(mu Au -> e Au)"

    def __init__(self) -> None:
        self.anchor = _load_l004_anchor(self.process_id)
        self.nuclear_inputs = mu_e_conversion_gold_nuclear_inputs()
    ...
    def _unevaluated_result(
        ...
        *,
        ...
        merged = {
            ...
            "passes_semantics": (
                "non-vetoing only; no CR(mu Au -> e Au) prediction was evaluated"
            ),
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        diagnostics.update(
            {
                ...
                "sm_prediction_policy": (
                    "Charged-LFV mu->e conversion has negligible SM rate; L004 "
        ...
        return ConstraintResult(
            ...
            notes=(
                "Pure-NP CR(mu Au -> e Au) from L001 dipole reuse plus "
                "documented scalar/vector nucleon coefficient proxies and "
                "gold nuclear overlap integrals; observed SINDRUM II Au limit "
                "from L004.yaml is a HARD veto. Ambiguous dipole-contact phase "
                "uses the destructive-interference lower envelope for the "
                "verdict and reports the full interval; scalar/vector matching "
                "is flagged NEEDS-HUMAN-PHYSICS."
            ),
