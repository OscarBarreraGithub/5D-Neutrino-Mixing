"""L004 - Muon-to-electron conversion in gold (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "R_mue^Au")) if isinstance(inner, dict) else "R_mue^Au"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_mue_conversion_placeholder("Au")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: L004 requires LFV/trident machinery not in core.",
