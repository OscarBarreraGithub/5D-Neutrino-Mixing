"""L008 - Tau to e gamma LFV dipole decay (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(tau->e gamma)")) if isinstance(inner, dict) else "BR(tau->e gamma)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_lfv_dipole_placeholder("tau->e gamma")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: L008 requires LFV/trident machinery not in core.",
