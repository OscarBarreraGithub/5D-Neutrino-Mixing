"""L008 - charged-LFV radiative tau decay ``tau -> e gamma``.

Physics
-------
The Standard Model charged-LFV rate is negligible for catalog purposes, so
L008 is a pure-new-physics dipole bound:

    BR_NP = prefactor_br * |(Y_N_bar Y_N_bar^dagger)_{e tau}|^2
            * (3 TeV / M_KK)^4.

The calculation reuses the L001 lepton dipole machinery in
``flavorConstraints.muToEGamma`` through the tau-e adapter
``flavor_catalog_constraints.physics_adapters.lepton_tau_e``.  The adapter
maps the L001 core's hardcoded ``(e,mu)`` element to the ``(e,tau)`` element by
...
proxy is not a loop-level RS ``tau -> e gamma`` calculation.

Catalog sidecar
---------------
``flavor_catalog/processes/charged_lepton/L008.yaml`` is the source of truth
for the PDG/BaBar branching-fraction limit and Belle II projection.  The
shared Perez-Randall NDA prefactor is loaded from the L001 sidecar, as in the
existing tau->mu radiative constraint that reuses the L001 dipole
normalization.
"""
...
_BELLE_CROSS_CHECK_CANDIDATES = ("recent_belle_cross_check",)
...
@dataclass(frozen=True)
class L008Anchor:
    """Typed L008 inputs: current tau->e gamma limit plus dipole prefactor."""

    experimental: Anchor
    primary_experiment: Anchor
    belle_cross_check: Anchor
    ...
    @property
    def primary_experiment_source_url(self) -> str | None:
        """BaBar full-data source URL for the current limit."""
        return self.primary_experiment.source_url

    @property
    def belle_cross_check_source_url(self) -> str | None:
        """Belle full-data cross-check source URL."""
        return self.belle_cross_check.source_url
...
def _load_l008_anchor(process_id: str) -> L008Anchor:
    ...
    belle_cross_check = load_anchor(
        process_id,
        family=_FAMILY,
        candidates=_BELLE_CROSS_CHECK_CANDIDATES,
    )
    ...
    _assert_positive_limit(
        belle_cross_check,
        process_id=process_id,
        label="recent_belle_cross_check",
    )
    _assert_positive_limit(
        belle_ii_projection,
        process_id=process_id,
    ...
    if dipole_prefactor.value <= 0.0:
        raise AnchorError("L008: L001 dipole prefactor must be positive")

    return L008Anchor(
        experimental=experimental,
        primary_experiment=primary_experiment,
        belle_cross_check=belle_cross_check,
...
@register
class Constraint:
    """Catalogued ``tau -> e gamma`` charged-LFV dipole constraint."""

    process_id = "L008"
    severity = Severity.HARD
    observable = "BR(tau -> e gamma)"

    def __init__(self) -> None:
        self.anchor = _load_l008_anchor(self.process_id)
    ...
    def _unevaluated_result(
        ...
        *,
        ...
        merged_diagnostics = {
            ...
            "passes_semantics": (
                "non-vetoing only; no BR(tau -> e gamma) prediction was evaluated"
            ),
            "sm_prediction_policy": (
                "Charged-LFV SM rate is negligible; L008 is applied as a "
                "pure-NP branching-fraction upper bound when proxy inputs exist."
            ),
            "budget_source": self.anchor.source_url,
            "primary_experiment_source": self.anchor.primary_experiment_source_url,
            "belle_cross_check_source": self.anchor.belle_cross_check_source_url,
            "experimental_block": self.anchor.experimental.block_key,
            "primary_experiment_block": self.anchor.primary_experiment.block_key,
            "belle_cross_check_limit": float(self.anchor.belle_cross_check.value),
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        try:
            result = tau_to_e_gamma_from_lepton_input(
                lepton_input,
        ...
        diagnostics.update(
            {
                "evaluated": True,
                "needs_human_physics": TAU_TO_E_GAMMA_PROXY_V1,
                ...
                "off_diagonal_13": complex(result.off_diagonal_13),
                "off_diagonal_31": complex(result.off_diagonal_31),
                ...
                "sm_prediction_policy": (
                    ...
                    "the pure NP tau->e dipole branching fraction."
                ),
                "budget_source": self.anchor.source_url,
                "primary_experiment_source": self.anchor.primary_experiment_source_url,
                "belle_cross_check_source": self.anchor.belle_cross_check_source_url,
        ...
        return ConstraintResult(
            ...
            notes=(
                "Pure-NP BR(tau -> e gamma) dipole check using the existing "
                "flavorConstraints.muToEGamma machinery through the tau-e "
                "lepton adapter. The HARD budget is the PDG/BaBar observed "
                "branching-fraction limit from L008.yaml; the tau-e dipole "
...
def _is_intentional_flavor_override(exc: Exception) -> bool:
    ...
    return isinstance(exc, ValueError) and (
        "tau->e gamma adapter is pinned to initial_flavor='tau'" in message
        or "tau->e gamma adapter is pinned to final_flavor='e'" in message
    )
