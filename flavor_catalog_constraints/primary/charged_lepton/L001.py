"""L001 - Muon to electron gamma LFV dipole decay (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(mu->e gamma)")) if isinstance(inner, dict) else "BR(mu->e gamma)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_lfv_dipole_placeholder("mu->e gamma")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: L001 requires LFV/trident machinery not in core.",
