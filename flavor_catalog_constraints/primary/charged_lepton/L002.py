"""L002 - Muon to three electrons LFV four-lepton decay (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(mu->3e)")) if isinstance(inner, dict) else "BR(mu->3e)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_lfv_4lepton_placeholder("mu->3e")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: L002 requires LFV/trident machinery not in core.",
