"""L003 - Muon-to-electron conversion in aluminum (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "R_mue^Al")) if isinstance(inner, dict) else "R_mue^Al"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_mue_conversion_placeholder("Al")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: L003 requires LFV/trident machinery not in core.",
