"""EW002 - First-row CKM unitarity Vud2+Vus2+Vub2 (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "First-row CKM unitarity Vud2+Vus2+Vub2")) if isinstance(inner, dict) else "First-row CKM unitarity Vud2+Vus2+Vub2"
...
class Constraint:
    ...
    observable = "First-row CKM unitarity Vud2+Vus2+Vub2"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("CKM unitarity")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: EW002 requires top/EW/LFV machinery not in core.",
