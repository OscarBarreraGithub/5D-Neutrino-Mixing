"""T006 - top FCNC decay ``t -> u g``.

Physics
-------
The observable is the branching fraction ``BR(t -> u g)``.  This is the
up-quark analog of T005 and deliberately reuses the shared top-FCNC
chromomagnetic-dipole machinery, ``quarkConstraints.top_fcnc``, reached only
through the ``flavor_catalog_constraints.physics_adapters.top_fcnc`` boundary.
The effective dipole convention is

    L = g_s G^a_mu ubar T^a i sigma^{mu nu} k_nu / m_t
...
``zeta_ut = (g_ut / g_s) * m_t^2 / M_KK^2``.  This is not a human-approved RS
...
95% CL upper limit loaded from T006.yaml.

Catalog sidecar
---------------
``flavor_catalog/processes/top_higgs_ew/T006.yaml`` is the source of truth for
the PDG/ATLAS/CMS limits and the Aguilar-Saavedra SM reference value.  T006
...
_ATLAS_TUG = "PDG2025:T006:ATLAS2022:t_ug"
_ATLAS_CUGUT = "PDG2025:T006:ATLAS2022:CuGut"
_ATLAS_UG_TO_T = "PDG2025:T006:ATLAS2022:ug_to_t_cross_section"
_CMS_TUG = "CMS2017:T006:t_ug"
_CMS_KAPPA_TUG = "CMS2017:T006:kappa_tug"
_SM_AGUILAR_SAAVEDRA = "AguilarSaavedra2004:T006:t_ug_SM"
...
_SCAFFOLD_UNCERTAINTY_KEY = "__t006_uncertainty_is_not_used__"
_BUDGET_POLICY = (
    "active budget is the numerically strongest t->u g branching-fraction "
    "95% CL upper limit in T006.yaml; ATLAS C_uG, ug->t, and older CMS "
    "normalizations are retained as context."
)
_T006_RS_GLUON_DIPOLE_PROXY_ASSUMPTION = (
    ...
    "t-u-g dipole is zeta_ut=(g_ut/g_s)*(m_t^2/M_KK^2), with no loop/Yukawa "
    "matching, SMEFT C_uG normalization, QCD running, or ug->t collider-recast "
...
class T006ValueAnchor:
    """Typed T006 value entry routed through the scaffold ``load_anchor``."""
...
@dataclass(frozen=True)
class T006Anchor:
    """All YAML-loaded T006 anchors used by the constraint."""

    atlas_tug: T006ValueAnchor
    atlas_cugut: T006ValueAnchor
    atlas_ug_to_t_cross_section: T006ValueAnchor
    cms_tug: T006ValueAnchor
    cms_kappa_tug: T006ValueAnchor
    sm_aguilar_saavedra: T006ValueAnchor
    active_limit: T006ValueAnchor
    budget_policy: str
...
def _required_float(value: Any, *, process_id: str, field_name: str) -> float:
    try:
        ...
        raise AnchorError(
            f"{process_id}: T006 field {field_name!r}={value!r} is not numeric"
        ) from exc
    if not math.isfinite(number):
        raise AnchorError(
            f"{process_id}: T006 field {field_name!r}={value!r} is not finite"
        )
    return number
    ...
    *,
    ...
    if scaffold_anchor.block_key != block_key:
        raise AnchorError(
            ...
            f"expected {block_key!r} for T006 value_id {value_id!r}"
        )
    return T006ValueAnchor(
...
def _load_t006_anchor(process_id: str) -> T006Anchor:
    atlas_tug = _load_limit_anchor(_ATLAS_TUG, process_id=process_id)
    atlas_cugut = _load_limit_anchor(
        _ATLAS_CUGUT,
        process_id=process_id,
        expected_units=_EXPECTED_CUG_UNITS,
    )
    atlas_ug_to_t = _load_limit_anchor(
        _ATLAS_UG_TO_T,
        process_id=process_id,
        expected_units=_EXPECTED_XSEC_UNITS,
    )
    cms_tug = _load_limit_anchor(_CMS_TUG, process_id=process_id)
    ...
    active = min((atlas_tug, cms_tug), key=lambda item: item.value)
    if active.value <= 0.0 or sm.value <= 0.0:
        raise AnchorError(f"{process_id}: T006 budget and SM estimate must be positive")
    return T006Anchor(
        atlas_tug=atlas_tug,
        atlas_cugut=atlas_cugut,
        atlas_ug_to_t_cross_section=atlas_ug_to_t,
        cms_tug=cms_tug,
        cms_kappa_tug=cms_kappa,
...
@register
class Constraint:
    """Catalogued ``BR(t -> u g)`` top-FCNC constraint (process_id T006)."""
    ...
    def __init__(self) -> None:
        self.anchor = _load_t006_anchor(self.process_id)
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        if couplings is None:
            return ConstraintResult(
                ...
                notes=(
                    f"extra {_REQUIRED_EXTRA!r} absent; t -> u g constraint "
                    "was not evaluated."
                ),
                diagnostics={
                    "missing_extra": _REQUIRED_EXTRA,
                    "needs_human_physics": _T006_RS_GLUON_DIPOLE_PROXY_ASSUMPTION,
                    "shared_top_fcnc_proxy_assumption": (
                        TOP_FCNC_RS_GLUON_DIPOLE_PROXY_ASSUMPTION_V1
                    ),
        ...
        diagnostics.update(
            {
                "matching_assumption": _T006_RS_GLUON_DIPOLE_PROXY_ASSUMPTION,
                "shared_top_fcnc_proxy_assumption": (
                    TOP_FCNC_RS_GLUON_DIPOLE_PROXY_ASSUMPTION_V1
                ),
                ...
                "atlas_tug_limit": float(self.anchor.atlas_tug.value),
                "atlas_cugut_limit_tev_minus2": float(self.anchor.atlas_cugut.value),
                "atlas_ug_to_t_cross_section_limit_pb": float(
                    self.anchor.atlas_ug_to_t_cross_section.value
                ),
                "cms_tug_limit": float(self.anchor.cms_tug.value),
                "cms_kappa_tug_limit_tev_minus1": float(
                    self.anchor.cms_kappa_tug.value
                ),
                "budget_policy": self.anchor.budget_policy,
                "operator_convention": TOP_FCNC_GLUON_DIPOLE_CONVENTION,
                "rs_matching_assumption": _T006_RS_GLUON_DIPOLE_PROXY_ASSUMPTION,
                "needs_human_physics": _T006_RS_GLUON_DIPOLE_PROXY_ASSUMPTION,
