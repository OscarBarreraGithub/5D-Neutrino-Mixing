    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "BR(h->eτ)")) if isinstance(inner, dict) else "BR(h->eτ)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        return ConstraintResult(
            ...
            notes="Evaluation deferred: T019 requires top/EW/LFV machinery not in core.",
