"""T010 - R_b, A_FB^b, A_b at Z pole (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "R_b, A_FB^b, A_b at Z pole")) if isinstance(inner, dict) else "R_b, A_FB^b, A_b at Z pole"
...
class Constraint:
    ...
    observable = "R_b, A_FB^b, A_b at Z pole"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("Z-bb pole")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: T010 requires top/EW/LFV machinery not in core.",
