    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "BR(t->uγ)")) if isinstance(inner, dict) else "BR(t->uγ)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("top FCNC radiative")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: T004 requires top/EW/LFV machinery not in core.",
