"""T017 - lepton-flavor-violating ``Z -> mu tau`` branching fraction.
...
fraction ``BR(Z -> mu+- tau-+)``. The rate reuses the shared Z-pole LFV
...
where the factor of two implements the charge-summed ``mu tau`` final state.
...
Catalog sidecar
---------------
``flavor_catalog/processes/top_higgs_ew/T017.yaml`` is the source of truth for
...
_ATLAS_DATASET_VALUE_ID = "ATLAS2021:T017:dataset"
_CMS_LIMIT_VALUE_ID = "CMS2025:T017:zmutau_limit"
_CMS_DATASET_VALUE_ID = "CMS2025:T017:dataset"
...
class LFVLimitEntry:
    """Typed view over one value entry in the T017 YAML ``values`` list."""
...
@dataclass(frozen=True)
class T017Anchor:
    """Typed T017 anchor bundle: canonical bound plus context limits."""

    experimental: Anchor
    pdg_limit: LFVLimitEntry
    atlas_combined_limit: LFVLimitEntry
    atlas_leptonic_limit: LFVLimitEntry
...
def _parse_limit_value(
    ...
    *,
    ...
    if isinstance(value, (int, float)):
        ...
        if match is None:
            raise AnchorError(
                f"{process_id}: T017 field {field_name!r}={value!r} is not "
                "a numeric upper-limit string"
            )
        number = float(match.group("number"))
    else:
        raise AnchorError(
            f"{process_id}: T017 field {field_name!r}={value!r} is not numeric"
        )
    if not math.isfinite(number) or number <= 0.0:
        raise AnchorError(f"{process_id}: T017 field {field_name!r} must be positive")
    return number
...
def _find_value_entry(
    ...
    raise AnchorError(
        f"{process_id}: value_id {value_id!r} not found in T017 values "
        f"(present: {present})"
    )
...
def _load_t017_anchor(process_id: str) -> T017Anchor:
    data = load_full_yaml(process_id, family=_FAMILY)
    pdg = data.get("pdg_or_equivalent")
    if not isinstance(pdg, Mapping):
        raise AnchorError(f"{process_id}: expected pdg_or_equivalent mapping")
    ...
    atlas_leptonic = _entry_view(
        process_id,
        _ATLAS_LEPTONIC_LIMIT_VALUE_ID,
        value_summary=value_summary,
    )
    ...
    return T017Anchor(
        ...
        atlas_leptonic_limit=atlas_leptonic,
        cms_limit=cms,
...
@register
class Constraint:
    """Catalogued ``Z -> mu tau`` LFV pure-NP branching-fraction constraint."""
    ...
    def __init__(self) -> None:
        self.anchor = _load_t017_anchor(self.process_id)
        ...
        diagnostics.update(
            {
                ...
                "delta_g_left_mutau": complex(result.delta_g_left),
                "delta_g_right_mutau": complex(result.delta_g_right),
                ...
                "atlas_leptonic_limit": float(self.anchor.atlas_leptonic_limit.limit),
                "atlas_leptonic_value_id": self.anchor.atlas_leptonic_limit.value_id,
