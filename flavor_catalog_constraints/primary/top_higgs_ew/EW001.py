"""EW001 - Peskin-Takeuchi STU (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "Peskin-Takeuchi STU")) if isinstance(inner, dict) else "Peskin-Takeuchi STU"
...
class Constraint:
    ...
    observable = "Peskin-Takeuchi STU"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("EW oblique")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: EW001 requires top/EW/LFV machinery not in core.",
