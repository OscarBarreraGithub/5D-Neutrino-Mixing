"""T011 - A_FB^b, A_LR^b combos (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "A_FB^b, A_LR^b combos")) if isinstance(inner, dict) else "A_FB^b, A_LR^b combos"
...
class Constraint:
    ...
    observable = "A_FB^b, A_LR^b combos"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("Z-bb asymmetries")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: T011 requires top/EW/LFV machinery not in core.",
