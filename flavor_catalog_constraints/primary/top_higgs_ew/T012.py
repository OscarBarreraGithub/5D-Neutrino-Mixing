"""T012 - R_c, A_FB^c, A_c at Z pole (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v; break
    obs = str(inner.get("observable", "R_c, A_FB^c, A_c at Z pole")) if isinstance(inner, dict) else "R_c, A_FB^c, A_c at Z pole"
...
class Constraint:
    ...
    observable = "R_c, A_FB^c, A_c at Z pole"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_top_higgs_ew_placeholder("Z-cc pole")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: T012 requires top/EW/LFV machinery not in core.",
