"""B023 - BR(B->K*(892) νν) (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(B->K*(892) νν)")) if isinstance(inner, dict) else "BR(B->K*(892) νν)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 invisible")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B023 requires B-sector machinery not in core.",
