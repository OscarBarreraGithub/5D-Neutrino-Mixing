"""B017 - Excl/incl b->s ll combo (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "Excl/incl b->s ll combo")) if isinstance(inner, dict) else "Excl/incl b->s ll combo"
...
class Constraint:
    ...
    observable = "Excl/incl b->s ll combo"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 rare semileptonic combo")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B017 requires B-sector machinery not in core.",
