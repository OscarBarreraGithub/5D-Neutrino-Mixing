"""B011 - BR(B->Xs γ) inclusive (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(B->Xs γ) inclusive")) if isinstance(inner, dict) else "BR(B->Xs γ) inclusive"
...
class Constraint:
    ...
    observable = "BR(B->Xs γ) inclusive"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B011 requires B-sector machinery not in core.",
