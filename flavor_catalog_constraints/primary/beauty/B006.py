    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(Bd->μμ)")) if isinstance(inner, dict) else "BR(Bd->μμ)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B006 requires B-sector machinery not in core.",
