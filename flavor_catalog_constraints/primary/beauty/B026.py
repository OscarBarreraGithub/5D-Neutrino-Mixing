"""B026 - charged-current LFU ratio ``R_D*`` in ``B -> D* tau nu``.

Physics
-------
The rigorous SM normalization is the HFLAV CKM-2025 SM reference in
``B026.yaml``.  This constraint does not hardcode the SM ratio; it loads the
YAML ``sm_reference_used_by_hflav.rdstar_value`` anchor and passes it into the
shared semileptonic-LFU core with ``mode="B->D*"``.  The v1 point-dependent
prediction is
...
``m_b m_tau / M_KK^2``.  This module reaches that core only through the
``flavor_catalog_constraints.physics_adapters.semileptonic_lfu`` boundary.

Severity
--------
HARD.  ``R_D*`` is an observed LFU ratio.  The HARD budget is built entirely
from ``B026.yaml`` as
``|R_D*^exp - R_D*^SM| + sqrt(sigma_exp^2 + sigma_SM^2)``.  The verdict
compares the total predicted ratio with the HFLAV average using that envelope,
matching the LFU-ratio treatment used by B025.

Catalog sidecar
---------------
``flavor_catalog/processes/beauty/B026.yaml`` is the source of truth for the
...
@dataclass(frozen=True)
class RDStarBudgetBand:
    """B026 measured-vs-SM envelope for the HARD verdict."""
...
@dataclass(frozen=True)
class B026Anchor:
    """Typed B026 anchor: HFLAV average, SM reference, and budget."""

    experimental: Anchor
    standard_model: Anchor
    joint_fit_context: RDStarJointFitContext
    budget_band: RDStarBudgetBand
    ...
    def uncertainty(self) -> float:
        if self.experimental.uncertainty is None:
            raise AnchorError("B026: experimental anchor lacks uncertainty")
...
def _required_float(value: Any, *, process_id: str, field_name: str) -> float:
    try:
        ...
        raise AnchorError(
            f"{process_id}: B026 anchor field {field_name!r}={value!r} is not numeric"
        ) from exc
    if not math.isfinite(number):
        raise AnchorError(
            f"{process_id}: B026 anchor field {field_name!r}={value!r} is not finite"
        )
    return number
...
def _positive_float(value: Any, *, process_id: str, field_name: str) -> float:
    ...
    if number <= 0.0:
        raise AnchorError(f"{process_id}: B026 anchor field {field_name!r} <= 0")
    return number


def _load_joint_context(process_id: str) -> RDStarJointFitContext:
    ...
    return RDStarJointFitContext(
        block_key=block_key,
        rd_value=_positive_float(
            sub.get("rd_value"),
            process_id=process_id,
            field_name=f"{block_key}.rd_value",
        ),
        rd_uncertainty=_positive_float(
...
def _build_budget_band(
    *,
    ...
    if experimental.uncertainty is None or experimental.uncertainty <= 0.0:
        raise AnchorError("B026: experimental uncertainty is required")
    if standard_model.uncertainty is None or standard_model.uncertainty <= 0.0:
        raise AnchorError("B026: SM uncertainty is required")
    ...
    if budget <= 0.0 or not math.isfinite(budget):
        raise AnchorError("B026: constructed R_D* budget is invalid")
    return RDStarBudgetBand(
        ...
        construction=(
            ...
            "B026.yaml HFLAV average and SM reference"
        ),
    )


def _require_anchor_block(anchor: Anchor, expected_block_key: str, *, process_id: str) -> Anchor:
    if anchor.block_key != expected_block_key:
        raise AnchorError(
            f"{process_id}: load_anchor selected {anchor.block_key!r}, "
            f"expected {expected_block_key!r}"
        )
    return anchor


def _load_b026_anchor(process_id: str) -> B026Anchor:
    experimental = load_anchor(
        process_id,
        family=_FAMILY,
        candidates=_EXPERIMENTAL_ANCHOR_CANDIDATES,
    )
    experimental = _require_anchor_block(
        experimental,
        _EXPERIMENTAL_ANCHOR_CANDIDATES[0],
        process_id=process_id,
    )
    ...
    standard_model = load_anchor(
        ...
        value_key="rdstar_value",
        uncertainty_key="rdstar_uncertainty",
    )
    standard_model = _require_anchor_block(
        standard_model,
        _SM_ANCHOR_CANDIDATES[0],
        process_id=process_id,
    )
...
def _budget_result(predicted: float, anchor: B026Anchor) -> tuple[float, float, bool]:
...
def _anchor_diagnostics(anchor: B026Anchor) -> dict[str, float | str | None]:
    return {
        ...
        "joint_fit_rd_value": float(anchor.joint_fit_context.rd_value),
        "joint_fit_rd_uncertainty": float(anchor.joint_fit_context.rd_uncertainty),
...
@register
class Constraint:
    """Catalogued ``R_D*`` charged-current LFU ratio constraint (B026)."""
    ...
    def __init__(self) -> None:
        self.anchor = _load_b026_anchor(self.process_id)
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        wilsons = semileptonic_lfu_wilson_proxy_from_couplings(
            couplings,
            m_kk_gev=None if kk_ew_mass is None else float(kk_ew_mass),
            inputs=self.sm_inputs,
        )
        result = semileptonic_lfu_ratio_from_scalar_shift(
            wilsons.scalar_amplitude_shift,
            inputs=self.sm_inputs,
        )
        ...
        diagnostics.update(
            {
                ...
                "sm_anchor_rdstar": float(self.anchor.sm_value),
                "sm_formula_rdstar": float(result.sm_ratio),
                "sm_formula_minus_anchor": float(result.sm_ratio - self.anchor.sm_value),
                "np_shift_rdstar": float(result.np_shift_ratio),
        ...
        return ConstraintResult(
            ...
            notes=(
                ...
                "The HARD budget is the B026.yaml measured-vs-SM envelope."
            ),
