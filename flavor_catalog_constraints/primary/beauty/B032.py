"""B032 - Bbar->π Kbar nonleptonic (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "Bbar->π Kbar nonleptonic")) if isinstance(inner, dict) else "Bbar->π Kbar nonleptonic"
...
class Constraint:
    ...
    observable = "Bbar->π Kbar nonleptonic"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 charmless 2-body")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B032 requires B-sector machinery not in core.",
