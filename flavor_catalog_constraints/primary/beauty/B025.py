"""B025 - R_D = BR(B->D τν)/BR(B->D lν) (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "R_D = BR(B->D τν)/BR(B->D lν)")) if isinstance(inner, dict) else "R_D = BR(B->D τν)/BR(B->D lν)"
...
class Constraint:
    ...
    observable = "R_D = BR(B->D τν)/BR(B->D lν)"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("LFU in b->c τν")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B025 requires B-sector machinery not in core.",
