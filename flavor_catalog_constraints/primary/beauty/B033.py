"""B033 - B->φ K_S penguin CPV (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "B->φ K_S penguin CPV")) if isinstance(inner, dict) else "B->φ K_S penguin CPV"
...
class Constraint:
    ...
    observable = "B->φ K_S penguin CPV"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 penguin CPV")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B033 requires B-sector machinery not in core.",
