"""B021 - Λ_b->Λ ll baryonic (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "Λ_b->Λ ll baryonic")) if isinstance(inner, dict) else "Λ_b->Λ ll baryonic"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 baryonic rare")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B021 requires B-sector machinery not in core.",
