"""B001 - Δm_d Bd mixing (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "Δm_d Bd mixing")) if isinstance(inner, dict) else "Δm_d Bd mixing"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("B0-B0bar mixing (ΔB=2)")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B001 requires B-sector machinery not in core.",
