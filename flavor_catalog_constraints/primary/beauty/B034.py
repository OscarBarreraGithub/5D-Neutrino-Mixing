"""B034 - Bs->φφ penguin (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "Bs->φφ penguin")) if isinstance(inner, dict) else "Bs->φφ penguin"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 penguin")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B034 requires B-sector machinery not in core.",
