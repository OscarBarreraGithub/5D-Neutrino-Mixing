    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(B+->τν)")) if isinstance(inner, dict) else "BR(B+->τν)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("ΔB=1 leptonic")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B009 requires B-sector machinery not in core.",
