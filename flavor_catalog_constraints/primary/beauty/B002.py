"""B002 - sin 2β = S_ψK_S (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "sin 2β = S_ψK_S")) if isinstance(inner, dict) else "sin 2β = S_ψK_S"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_beauty_placeholder("CP phase in Bd mixing")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: B002 requires B-sector machinery not in core.",
