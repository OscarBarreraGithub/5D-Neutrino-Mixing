"""C003 - Direct CP violation in D0 -> K+K- / pi+pi- (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "A_CP^{KK} - A_CP^{pi pi}")) if isinstance(inner, dict) else "A_CP^{KK} - A_CP^{pi pi}"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_charm_placeholder("ΔC=1 direct CPV")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C003 requires charm-sector NP machinery not in core.",
