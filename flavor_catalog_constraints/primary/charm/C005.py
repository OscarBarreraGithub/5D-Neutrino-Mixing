"""C005 - Rare leptonic decay D0 -> e+ e- (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(D0->ee)")) if isinstance(inner, dict) else "BR(D0->ee)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        ...
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C005 requires charm-sector NP machinery not in core.",
