"""C002 - CP violation in neutral-charm mixing (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "|q/p|, phi_D")) if isinstance(inner, dict) else "|q/p|, phi_D"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_charm_placeholder("ΔC=2 CP-violating phase")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C002 requires charm-sector NP machinery not in core.",
