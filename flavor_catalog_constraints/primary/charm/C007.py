"""C007 - Rare semileptonic D+ -> pi+ mu+ mu- (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(D+->pi+ mumu)")) if isinstance(inner, dict) else "BR(D+->pi+ mumu)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_charm_placeholder("ΔC=1 rare semileptonic")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C007 requires charm-sector NP machinery not in core.",
