"""C008 - LFV semileptonic D+ -> pi+ e mu (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(D+->pi+ e mu)")) if isinstance(inner, dict) else "BR(D+->pi+ e mu)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_charm_placeholder("ΔC=1 LFV semileptonic")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C008 requires charm-sector NP machinery not in core.",
