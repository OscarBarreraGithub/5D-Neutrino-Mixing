"""C001 - Neutral charm mixing (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "x_D, y_D, Delta m_D")) if isinstance(inner, dict) else "x_D, y_D, Delta m_D"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_charm_placeholder("D0-Dbar0 mixing (ΔC=2)")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C001 requires charm-sector NP machinery not in core.",
