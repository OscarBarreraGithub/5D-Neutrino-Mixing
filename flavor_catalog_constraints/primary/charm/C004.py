"""C004 - Rare leptonic decay D0 -> mu+ mu- (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            if isinstance(v, dict) and ("observable" in v or "source" in v):
                inner = v
                break
    obs = str(inner.get("observable", "BR(D0->mumu)")) if isinstance(inner, dict) else "BR(D0->mumu)"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_charm_placeholder("ΔC=1 rare leptonic")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: C004 requires charm-sector NP machinery not in core.",
