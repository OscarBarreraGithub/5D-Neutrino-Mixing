"""CR010 - VLQ doublet (T,B) pair (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "VLQ doublet (T,B) pair")) if isinstance(inner, dict) else "VLQ doublet (T,B) pair"
...
class Constraint:
    ...
    observable = "VLQ doublet (T,B) pair"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_collider_rs_placeholder("VLQ doublet pair")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: CR010 requires direct-search recast machinery not in core.",
