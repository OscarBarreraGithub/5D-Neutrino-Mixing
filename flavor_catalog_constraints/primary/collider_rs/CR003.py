"""CR003 - T (T') custodial pair (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "T (T') custodial pair")) if isinstance(inner, dict) else "T (T') custodial pair"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_collider_rs_placeholder("VLQ T pair")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: CR003 requires direct-search recast machinery not in core.",
