"""CR008 - Singlet VLQ T pair (non-custodial) (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "Singlet VLQ T pair (non-custodial)")) if isinstance(inner, dict) else "Singlet VLQ T pair (non-custodial)"
...
class Constraint:
    ...
    observable = "Singlet VLQ T pair (non-custodial)"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_collider_rs_placeholder("VLQ singlet pair")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: CR008 requires direct-search recast machinery not in core.",
