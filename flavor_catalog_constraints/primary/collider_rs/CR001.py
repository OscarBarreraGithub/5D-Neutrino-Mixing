"""CR001 - KK-gluon in t-tbar (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "KK-gluon in t-tbar")) if isinstance(inner, dict) else "KK-gluon in t-tbar"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_collider_rs_placeholder("KK-gluon resonance")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: CR001 requires direct-search recast machinery not in core.",
