...
carry the EW KK mass but not that collider signal model, so CR005 uses the
documented benchmark mass-exclusion proxy.  The catalogued sigma*BR limits are
loaded and reported only as diagnostics.

Severity
--------
HARD.  The active budget is the CMS 2021 95% CL combined ee+mumu SSM-Z'
benchmark lower mass limit loaded from ``CR005.yaml``.  The ratio is
...
_BUDGET_POLICY = (
    "active HARD budget is the CMS2021 combined ee+mumu SSM-Z' benchmark "
    "mass lower limit in CR005.yaml. ATLAS and PDG sigma*BR upper limits are "
    "diagnostics only because ParameterPoint does not provide the RS "
    "production, branching-fraction, width, and acceptance model."
)
...
@register
class Constraint:
    """Catalogued neutral electroweak KK dilepton resonance mass constraint."""
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        try:
            ...
            return ConstraintResult(
                ...
                notes=f"invalid neutral-EW KK mass input; CR005 not evaluated: {exc}",
                diagnostics={
        ...
        return ConstraintResult(
            ...
            notes=(
                ...
                "mass lower bound. HARD ratio is m_limit/m_KK_EW; full "
