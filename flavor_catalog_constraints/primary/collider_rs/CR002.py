"""CR002 - T_{5/3} custodial pair (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "T_{5/3} custodial pair")) if isinstance(inner, dict) else "T_{5/3} custodial pair"
...
class Constraint:
    ...
    observable = "T_{5/3} custodial pair"
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_collider_rs_placeholder("VLQ exotic-charge pair")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: CR002 requires direct-search recast machinery not in core.",
