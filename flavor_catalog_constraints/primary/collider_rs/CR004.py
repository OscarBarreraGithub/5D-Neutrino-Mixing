"""CR004 - B custodial pair (placeholder)."""
    ...
    if isinstance(raw, dict):
        for v in raw.values():
            ...
            if isinstance(v, list) and v and isinstance(v[0], dict):
                inner = v[0]; break
    obs = str(inner.get("observable", "B custodial pair")) if isinstance(inner, dict) else "B custodial pair"
...
class Constraint:
    ...
    def evaluate(self, point: ParameterPoint) -> ConstraintResult:
        result = evaluate_collider_rs_placeholder("VLQ B pair")
        return ConstraintResult(
            ...
            notes="Evaluation deferred: CR004 requires direct-search recast machinery not in core.",
