...
ledger (the $(2/3)$-matrix-element / $1/6$-Wilson convention, cross-checked
against the literature to machine precision, that must never be mixed with the
textbook $8/3$ convention). An annotated reading guide closes the note.
\end{abstract}
...
\;\approx\; 11 ,
\qquad \text{then QCD running} \;\times\; \sim 8
\;\Rightarrow\; \sim 10^2 ,
\]
where the bare chiral factor $r_\chi\approx26$ \eqref{eq:chiral} enters the LR
matrix element through the colour/Fierz combination $\tfrac16 r_\chi+\tfrac14$
\eqref{eq:O4ME}, not as $r_\chi$ itself (the partner $\langle\QLRfive\rangle/%
\langle\QVLL\rangle\approx24$ is even larger). So the LR operator, although born
...
\begin{align}
\langle O_1^{\mathrm{VLL}}\rangle &= \tfrac{2}{3}\,f_K^2 m_K\, B_1^K,
\label{eq:O1ME}\\
\langle O_4^{\mathrm{LR}}\rangle &= \big(\tfrac16\,r_\chi + \tfrac14\big)\,f_K^2 m_K\, B_4^K,
\label{eq:O4ME}\\
\langle O_5^{\mathrm{LR}}\rangle &= \big(\tfrac12\,r_\chi + \tfrac{1}{12}\big)\,f_K^2 m_K\, B_5^K.
\label{eq:O5ME}
\end{align}
...
\S\ref{sec:basis}. The prefactor $\tfrac{2}{3}$ in \eqref{eq:O1ME} is the
load-bearing convention choice discussed next; it pairs with the $1/6$ in the
Wilson coefficient \eqref{eq:codeC1} (colour factor carried in the Wilson, not in
the matrix element).
...
\section{The normalisation convention --- and the factor-of-two that nearly broke it}
\label{sec:r5}

This is the single most important subtlety in the whole pipeline, and it is
recorded in the orchestration ledger as ``R5'' (a $\Delta F=2$ factor-2 scare that
resolved to \emph{the code is correct}). The hazard: there are two internally
consistent conventions for splitting a colour factor between the Wilson
coefficient and the matrix element, and \emph{mixing them gives a factor of two
wrong}.
\begin{itemize}[leftmargin=1.4em]
...
$\half$ (propagator) $\times\,\tfrac13$ (colour Fierz), and the matrix element is
$\langle O_1\rangle = \tfrac{2}{3} f_K^2 m_K B_1$ \eqref{eq:O1ME}.
\item \textbf{The textbook convention} carries the colour factor in the
\emph{matrix element}: $\langle O_1\rangle = \tfrac{8}{3} f_K^2 m_K B_1$ with a
correspondingly different Wilson normalisation.
\end{itemize}
Both are correct \emph{in isolation}; the danger is using a textbook-$8/3$ matrix
element with a repo-$1/6$ Wilson, or vice versa, which double-counts (or
half-counts) the colour. The ledger records the resolution: the repo's
$2|\Mtwelve^{\rm NP}|$ was compared directly to the published KK-gluon formula of
Csaki--Falkowski--Weiler (0804.1954, Eqs.~3.3--3.7, with the exact $SU(3)$ Fierz
$\sum_A T^A\otimes T^A=\half(\delta\otimes\delta-\tfrac1{N_c}\delta\otimes\delta)$)
and gave a ratio of $1.0000$ \emph{to machine precision}. A cross-check against
Blanke et al.\ (0809.1073, Eq.~4.33) gave $\sim1.69$, attributed to a
basis/running difference, explicitly ``not the $0.5\times$ failure mode.''

\begin{callout}{Convention hazard --- do not ``fix'' the $1/6$.}
...
$1/6$-Wilson convention is verified against the published RS formula to machine
precision. Never mix it with the textbook $8/3$-matrix-element convention. The
constraints \code{K001}/\code{K002}/\code{B001}/\code{B003}/\code{C001} and the
$\Delta F=2$ core are all correctly normalised in the repo convention; the
isolated matching path
\code{quarkConstraints/paper\_0710\_1869/eft\_deltaf2/matching\_kkgluon.py}
(line~523, \code{q1\_vll=...*(prefactor/6.0)}) uses the \emph{same} $1/6$ and is
test/script-only, so no live constraint mixes conventions. If you find yourself
about to insert a factor of two here, stop and re-read this box.
\end{callout}
...
$f_K=0.1557$, the $1/6$/$\tfrac23$ convention, $B_4^K=0.903$, $B_5^K=0.691$) are
