...
$(2/3)$-matrix-element / $(1/6)$-Wilson convention --- and \emph{why} it equals
the standard published normalization to machine precision. Part~\ref{part:subtle}
...
split of an $SU(3)$ colour factor between the Wilson coefficient and the hadronic
matrix element. Our code carries the colour factor in the Wilson (a $1/6$ on the
vector operator) and uses a $(2/3)$ matrix element; the textbook carries it in
the matrix element (an $8/3$ matrix element) with a different Wilson. \emph{Both
are correct in isolation; mixing them is a factor-of-two bug.} This nearly broke
the pipeline once (the ``R5 factor-2 scare''), and was resolved only by an
independent, convention-free cross-check against the published RS formula. That
resolution lives in \S\ref{sec:r5}, and it is the reason this note is tagged the
authoritative reference: any future edit to a meson note that touches a factor
...
= \frac{\big(\tfrac16 r_\chi+\tfrac14\big)B_4^K}{\tfrac23 B_1^K}
\approx 11 ,
\qquad\Big(\text{and }\frac{\langle\QLRfive\rangle}{\langle\QVLL\rangle}\approx24\Big),
\]
then QCD running multiplies the LR piece by $\sim8$ (\S\ref{sec:running}):
\[
11\times 8 \;\approx\; \sim 10^2 .
\]
...
\begin{align}
\langle O_1^{\mathrm{VLL}}\rangle &= \tfrac{2}{3}\,f_P^2 m_P\, B_1,
\label{eq:O1ME}\\
\langle O_4^{\mathrm{LR}}\rangle &= \big(\tfrac16\,r_\chi + \tfrac14\big)\,f_P^2 m_P\, B_4,
\label{eq:O4ME}\\
\langle O_5^{\mathrm{LR}}\rangle &= \big(\tfrac12\,r_\chi + \tfrac{1}{12}\big)\,f_P^2 m_P\, B_5.
\label{eq:O5ME}
\end{align}
The code lines are \code{me\_vll = (2.0/3.0)*fp2\_mp*B\_1},
\code{me\_lr4 = (r\_chi/6.0 + 0.25)*fp2\_mp*B\_4}, and
\code{me\_lr5 = (r\_chi/2.0 + 1.0/12.0)*fp2\_mp*B\_5}
(\code{deltaf2.py}:910--912; the kaon copy at lines~722--724). The chiral factor
...
\end{tabular}
\end{center}
The $\tfrac23$ prefactor in \eqref{eq:O1ME} (literally \code{me\_vll = (2/3)\dots})
is the load-bearing convention choice: it pairs with the $1/6$ in the Wilson
\eqref{eq:codeC1} so that the colour factor is carried in the Wilson, not in the
matrix element. See \S\ref{sec:r5}.
...
\section{The normalization convention --- and the factor-of-two that nearly broke it}
\label{sec:r5}

This is the single most important subtlety in the whole pipeline, recorded in the
orchestration ledger (\code{.orchestration/PHASE2\_PROGRAM\_LEDGER.md}) as the
``R5'' item --- a $\Delta F=2$ factor-2 scare that resolved to \emph{the code is
correct}. The hazard is that there are \emph{two} internally consistent
conventions for splitting an $SU(3)$ colour factor between the Wilson coefficient
and the matrix element, and \emph{mixing them gives a factor of two wrong}:
\begin{itemize}[leftmargin=1.4em]
...
$\half$ (propagator) $\times\,\tfrac13$ (colour Fierz, from \eqref{eq:fierz}), and
the matrix element is $\langle O_1\rangle = \tfrac{2}{3} f_P^2 m_P B_1$
\eqref{eq:O1ME}.
\item \textbf{The textbook convention} carries the colour factor in the
\emph{matrix element}: $\langle O_1\rangle = \tfrac{8}{3} f_P^2 m_P B_1$ with a
correspondingly different (larger) Wilson normalization. The textbook ratio
$\tfrac83/\tfrac23 = 4 = \Nc + 1/\Nc\cdot\dots$ is exactly the colour factor moved
from one side to the other.
\end{itemize}
Both are correct \emph{in isolation}. The danger is using a textbook-$\tfrac83$
matrix element with a repo-$1/6$ Wilson (or vice versa), which double-counts or
half-counts the colour and shifts $\Mtwelve^{\rm NP}$ by exactly two.

\paragraph{How R5 was resolved (the convention-free cross-check).} Coefficient-by-%
coefficient reconstruction proved unreliable --- reviewers quoted
$\tfrac13,\tfrac23,\tfrac43,\tfrac83$ for $\langle O_1\rangle$ across different
runs, and the vote split. The resolution was to bypass the coefficient bookkeeping
entirely and compare a \emph{physical} quantity to a \emph{published} RS formula.
Per the ledger's ``R5 RESOLVED'' section (2026-06-03, dual-confirmed):
\begin{itemize}[leftmargin=1.4em]
\item the repo's $2|\Mtwelve^{\rm NP}|$ was compared directly to the KK-gluon
$\Delta F=2$ formula of Csaki--Falkowski--Weiler (0804.1954, Eqs.~3.3--3.7), with
the exact $SU(3)$ Fierz $\sum_A\TA\otimes\TA=\half(\delta\otimes\delta
-\tfrac1{\Nc}\delta\otimes\delta)\to(\tfrac13)Q_1$, and gave a
\textbf{ratio of $1.0000$ to machine precision};
\item an independent cross-check against Blanke et al.\ (0809.1073, Eq.~4.33) gave
$\sim1.69$, attributed to a basis/running difference and explicitly flagged as
``\emph{NOT the $0.5\times$ failure mode; $\times2$ would be the WRONG fix}.''
\end{itemize}
The conclusion, verbatim in spirit: deltaf2's $1/6$ Wilson factor is
$\half(\text{propagator})\times\tfrac13(\text{colour-Fierz})$;
$\langle O_1\rangle=\tfrac23 f_P^2 m_P B_1$ is the \emph{correct} matrix element
\emph{in that convention} (colour factor in the Wilson, not the matrix element).
The ``half'' verdicts had wrongly imposed the textbook $\tfrac83$/$\tfrac43$
convention. The core and the five anchored constraints
(\code{K001},\code{K002},\code{B001},\code{B003},\code{C001}, plus
\code{B002},\code{C002}) are all correctly normalized. \textbf{NO FIX.}

\begin{callout}{Documentation hazard --- do not ``fix'' the $1/6$ (ledger R5).}
The ledger entry is blunt: ``\emph{DO NOT ``fix'' this --- a $\times2$ would BREAK
agreement with the literature.}'' The repo's $(2/3)$-matrix-element /
$(1/6)$-Wilson convention is verified against the published RS formula
(0804.1954) to machine precision. \textbf{Never mix it with the textbook
$(8/3)$-matrix-element convention.} The constraints
\code{K001}/\code{K002}/\code{B001}/\code{B003}/\code{C001} and the $\Delta F=2$
core are all in the repo convention; the isolated matching path
\code{quarkConstraints/paper\_0710\_1869/eft\_deltaf2/matching\_kkgluon.py}
(line~523) uses
the \emph{same} $1/6$ and is test/script-only, so no live constraint mixes
conventions. If you find yourself about to insert a factor of two anywhere in the
$M_{12}^{\rm NP}$ chain, stop and re-read this box. This is the central reason this
note is the authoritative normalization reference for the four meson notes.
\end{callout}
...
constants ($1/6$ and $2/3$ colour split, $r_\chi$, the bag parameters,
