...
- **IN FLIGHT: W6b** — full-catalog harness driver (`scripts/run_full_catalog_scan.py`) + a 1e4-point SMOKE scan. codex task **`b4euiylsh`**, output → `.orchestration/runs/W6-HARNESS/impl6b_out.md`. ON RESUME: (1) read W6b out + the smoke results; (2) DUAL-REVIEW W6b (codex review ∥ Opus review) → route fixes → BOTH APPROVE → commit; (3) **present smoke results (post-cache s/point, 1e8 extrapolation, which constraints bite rigorous-vs-proxy, exception rate, universal-c SM sanity) to the user for go/no-go on the 100M run.**
- **NEXT after W6b:** the 100M scan (gated on smoke + user go). Then: **W5** exclusive form factors B013/B014 (DEFERRED — needs user sign-off on source; default Bharucha-Straub-Zwicky LCSR; secondary, stays PARTIAL); **R7/R8** doc dual-review (last-week plan + triage + status docs); **P7** loop residual (dipoles/EDMs) = genuine NEEDS-HUMAN, DOCUMENT only (no build).
