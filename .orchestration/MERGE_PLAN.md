...
**Granularity check (round 2)**: Code units (R01-R09) average ~5 commits, R10a/b/c are 9-11 commits each, R11-R19 are ~20-35 commits with structured Writer-Checker-Opus internal gating, website units 5-8 commits. R19 is now correctly 34 commits (R03 commit `82a96f0` is excluded by design; missing `82daa9b` restored per C18). Each unit fits within one Opus run.

**Retroactive corrections (C18, 2026-05-26)** — bookkeeping notes that do not alter the SHA list itself:
- **R12 row** (dispatch-prompt drift, R12-I1): the original R12 dispatch prompt described "L008 (mu->e conversion: SINDRUM-II Au target Br < 7e-13)" and "W4 = 12+ new procs". The actual W4 PKA surface is 20 processes; L008 is `tau- -> e- gamma` and the SINDRUM-II Au limit lives at L004. The R12 description above ("12+ new processes") is preserved as the original wording; the canonical scope is 20 W4 PKAs (adds B004, B006, EW003, C003, C004, C007, E004 on top of those enumerated).
- **R12 commit bundling notes** (R12-I2, R12-I3): commit `7d3da08` ("PKA draft for EW001 oblique parameters") additionally introduces the full E004 PKA pair (`processes/edm_neutrino/E004.{yaml,tex}` + refs + worklog); commit `bcd8907` ("Opus signoff on L001 -- APPROVE-OVERRIDE") additionally adds the W4 kaon_edm CA cycle-1 verdict (E004/E006/E008/K017 `CHECKER-DONE` transitions) and the `ca_w4_kaon_edm.md` worklog. Both are workflow-seam irregularities; no physics defect.
- **R13 row** (dispatch-prompt drift, R13-I1): the original R13 dispatch prompt named K010/B023/E007 as spot-check anchors with stale process descriptions; the canonical referents are K010 = `K_S → π⁰ e⁺e⁻`, B023 = `B → K*(892) ν ν̄`, E007 = `²²⁵Ra / ¹²⁹Xe atomic EDMs`. R13 reviewer re-routed to the actual catalog content and verified all three. Catalog itself is consistent.
- **R16 row** (B012 disposition framing, R16-I3): B012 was originally in DA-4's deferred list (not the "10 unallocated"). The Wave-7 deferred-scope addendum (`flavor_catalog/worklogs/discovery/round_004_addendum_deferred_scope.md:8-16`) and `flavor_catalog/audits/wave7_deferred_scope_review.md:13` re-promote it to ACTIVE on external-review grounds. Preferred wording for future write-ups: "DA-4 converged at 75; Wave-7 amended active coverage to 80 (4 unallocated + 1 previously-deferred B012 promoted on external-review grounds)."
...
- **R19 (collider_rs)**: each `CR0XX` ID, the `limit_type` field (under `pdg_or_equivalent.values[]`; the planner-era guess `claim_level` is not in the schema), the `pdg_or_equivalent` field, the SHA-256 listed in each `references/CR0XX/sha256sums.txt`.
